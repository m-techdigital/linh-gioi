...
def main() -> int:
    ...
    require(
        "client/Unity/Assets/Game/UI/Runtime/RuntimeViewportMetrics.cs",
        "LGO Runtime Viewport Metrics Canonical Owner v1",
        "internal static RuntimeViewportMetrics FromRoot(VisualElement root, string forcedProfile = null)",
        "ConvertSafeAreaToPanel",
        "screenHeight - safeAreaPixels.yMax",
        "internal bool LayoutEquals(RuntimeViewportMetrics other)",
        "internal string DebugSummary()",
    )
    require(
        "client/Unity/Assets/Game/UI/Runtime/RuntimePanelSettingsProvider.cs",
        "LGO Runtime Panel Settings Provider v1",
        "Resources.Load<PanelSettings>(ResourceName)",
    ...
    require(
        ...
        "_lastLayoutWidth == width && _lastLayoutHeight == height && _viewportMetrics.LayoutEquals(viewport)",
        "_lastLayoutWidth = width;",
        "_lastLayoutHeight = height;",
        "return RuntimePanelSettingsProvider.LoadOrCreate();",
        "_lastLayoutWidth == width && _lastLayoutHeight == height",
