...
TWO_D_GATES: list[dict[str, Any]] = [
    ...
    },
    {
        ...
        "contains": {
            ...
            "runtimeLinhThanhDistrictPreviewSnapshot": ["DistrictPreviewRail", "unlocked=True", "selected=spirit-temple", "label=Đền Linh", "route=plaza->spirit-temple", "controls=M select-district", "safe-no-buff-backend", "safe-no-district-backend", "safe-local-no-backend"],
            "runtimeLinhThanhDistrictDetailSnapshot": ["DistrictDetail", "selected=spirit-temple", "role=story-blessing-preview", "detail=altar-local-only", "next=quest-buff-gate", "safe-no-buff-backend", "safe-no-district-backend", "safe-local-no-backend"],
