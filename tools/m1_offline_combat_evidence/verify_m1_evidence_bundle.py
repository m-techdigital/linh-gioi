...
M1_TEST_NAMES = {
    ...
    'OfflineCombatHudBindsResultWithoutCombatSideEffects',
}
...
def assert_editmode_results(xml_text: str) -> None:
    ...
    missing = sorted(name for name in M1_TEST_NAMES if not any(name in candidate for candidate in names))
    if missing:
        raise SystemExit(f'ERROR: M1 EditMode result XML is missing expected tests: {missing}')
...
def main() -> int:
    ...
    print('M1_EVIDENCE_BUNDLE_VERIFY_PASS')
    return 0
