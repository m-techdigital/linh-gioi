...
import argparse, hashlib, json, sys, zipfile, tarfile
...
def check_sha(path: Path, sha_path: Path) -> None:
    if not sha_path.exists():
        raise SystemExit(f'ERROR: sha file missing: {sha_path}')
...
def main() -> int:
    ...
    with zipfile.ZipFile(args.evidence_zip) as zf:
        ...
        editmode = zf.read('unity-editmode-results.xml').decode('utf-8', 'replace')
        if 'test-case' not in editmode:
            raise SystemExit('ERROR: Unity EditMode results contain no test-case entries')
    ...
    print('UNITY_EVIDENCE_BUNDLE_VERIFY_PASS')
    return 0
