...
OUT_DIR="$ROOT/build/m4-playable-vertical-slice"
API_PORT="18085"
UNITY_PLAYER=""
...
usage() {
  cat <<'USAGE'
Usage:
  ./tools/run_m4_playable_vertical_slice_once.sh [--output-dir DIR] [--port PORT] [--unity-player PATH]

Runs the M4 playable vertical slice smoke against a real Spring Boot API process
and a current Unity player executable built from this source.
...
python3 tools/m4_playable_vertical_slice_runtime.py "${args[@]}"
