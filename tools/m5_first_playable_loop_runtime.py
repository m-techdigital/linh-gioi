...
def resolve_player(raw: str | None) -> Path | None:
    for value in [raw, os.environ.get('LGO_M5_UNITY_PLAYER')]:
        if not value:
            continue
...
def bundle(output_dir: Path, log: Path, store_dir: Path) -> Path:
    ...
    archive = output_dir / f'lgo-m5-first-playable-loop-smoke-{timestamp}.tar.gz'
...
def run_unity_smoke(root: Path, player: Path, base_url: str, result_json: Path, log: Path) -> None:
    command = [
        ...
        '--lgo-m5-api-url', base_url,
        '--lgo-m5-result', str(result_json),
        '--lgo-m5-dev-key', 'm5-first-playable-loop-dev-key',
        '--lgo-m5-character-name', 'M5LoopHero',
        '--lgo-m5-class-id', 'class.sword',
    ]
    ...
    if not result_json.exists():
        append(log, f'UNITY_M5_RESULT_MISSING path={result_json}')
        raise SystemExit(41)
...
def main() -> int:
    parser = argparse.ArgumentParser(description='Run the M5 first playable loop Unity player smoke.')
    ...
    log = output_dir / 'm5-first-playable-loop-runtime.log'
    ...
    append(log, '== Linh Gioi Online M5 first playable loop runtime smoke ==')
    ...
    if player is None:
        append(log, 'UNITY_RUNTIME_GATE=UNVERIFIED_ENVIRONMENT')
        append(log, 'REASON=no executable M5 Unity player was provided via --unity-player or LGO_M5_UNITY_PLAYER')
    ...
    try:
        ...
        run_unity_smoke(root, player, base_url, output_dir / 'unity-m5-first-playable-loop.json', log)
    finally:
    ...
    if not store_file.exists():
        ...
        return 45
    if 'm5-first-playable-loop-dev-key' in store_file.read_text(encoding='utf-8', errors='replace'):
        append(log, 'ERROR: raw M5 dev key leaked into persistence file.')
        return 46

    archive = bundle(output_dir, log, store_dir)
    append(log, f'M5_FIRST_PLAYABLE_LOOP_RUNTIME_SMOKE_PASS output={output_dir} bundle={archive}')
    return 0
