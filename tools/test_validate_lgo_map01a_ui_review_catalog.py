...
class ValidateMap01AUiReviewCatalogTests(unittest.TestCase):
    ...
    def _fixture(self) -> tempfile.TemporaryDirectory[str]:
        ...
        for manifest_rel, png_rel, width, height in validator.REGISTER_EVIDENCE:
            write_json(root / manifest_rel, {
                "status": "TECHNICAL_PASS_VISUAL_REVIEW_REQUIRED",
    ...
    def test_rejects_register_capture_without_expected_overlay(self) -> None:
        with self._fixture() as temp:
            manifest = Path(temp) / "build/map01a-register-runtime-v1/pc/manifest.json"
            data = json.loads(manifest.read_text())
            data["registerOverlayExpected"] = False
            write_json(manifest, data)
