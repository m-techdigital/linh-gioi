...
owned=np.zeros((h,w),np.float32);owned[:,2048:]=1;owned[:576,:1152]=1;owned[192:768,1152:1728]=1;owned[576:1152,576:1152]=1;gold*=np.clip(((r-b)/(r+.01)-.35)/.2,0,1)
gold*=owned
out=np.zeros_like(c);out[:,:,0]=gold*.58;out[:,:,3]=.17+gold*.43
...
print('LGO_WARDROBE_SURFACE_MASK_READY size=1024x512 metallic_max=0.58 smoothness=0.17..0.60')
