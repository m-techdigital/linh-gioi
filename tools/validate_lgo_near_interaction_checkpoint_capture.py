#!/usr/bin/env python3
from __future__ import annotations

import subprocess
import sys
from pathlib import Path


ROOT = Path(__file__).resolve().parents[1]
ERRORS: list[str] = []


def read(rel: str) -> str:
    path = ROOT / rel
    if not path.is_file():
        ERRORS.append(f"missing file: {rel}")
        return ""
    return path.read_text(encoding="utf-8", errors="replace")


def require(rel: str, *markers: str) -> None:
    text = read(rel)
    for marker in markers:
        if marker not in text:
            ERRORS.append(f"{rel} missing marker: {marker}")


def check_frozen() -> None:
    result = subprocess.run(
        [
            "git",
            "--no-pager",
            "diff",
            "--name-only",
            "--",
            "protocol",
            "gamedata/schemas",
            "docs/adr",
            "client/Unity/Assets/Game/UI/design-tokens.json",
        ],
        cwd=ROOT,
        text=True,
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        check=False,
    )
    if result.returncode != 0:
        ERRORS.append(result.stderr.strip() or "git frozen diff failed")
    elif result.stdout.strip():
        ERRORS.append("frozen surface changed")


def main() -> int:
    require(
        "client/Unity/Assets/Game/UI/Runtime/VisualRuntimeEvidenceRunner.cs",
        "near-gatekeeper-prompt.png",
        "near-training-stone-prompt.png",
        "Near Gate Keeper Prompt",
        "Near Training Stone Prompt",
        "CaptureEvidenceNearGateKeeperPrompt",
        "CaptureEvidenceNearTrainingStonePrompt",
    )
    require(
        "client/Unity/Assets/Game/UI/Runtime/M4PlayableClientController.cs",
        "CaptureEvidenceNearGateKeeperPrompt",
        "CaptureEvidenceNearTrainingStonePrompt",
        "SetSmokePositionNearGateKeeper",
        "SetSmokePositionNearTrainingStone",
    )
    require(
        "client/Unity/Assets/Game/World/Runtime/PlayableWorldController.cs",
        "LGO World Hub Interaction Readability v1",
        "characterSize = mobile ? 0.060f",
        "CurrentInteractionPromptOffset",
        "CurrentInteractionPromptPosition",
        "CurrentGateKeeperVisualPosition()",
    )
    require(
        "tools/lgo_visual_runtime_review.sh",
        "near-gatekeeper-prompt.png",
        "near-training-stone-prompt.png",
    )
    require(
        "tools/analyze_lgo_visual_runtime_evidence.py",
        "near-gatekeeper-prompt.png",
        "near-training-stone-prompt.png",
    )
    require(
        "tools/report_lgo_visual_evidence_profile_index.py",
        "near-gatekeeper-prompt.png",
        "near-training-stone-prompt.png",
    )
    require(
        "docs/tasks/LGO-NEAR-INTERACTION-CHECKPOINT-CAPTURE-PASS-v1.0.md",
        "LGO_NEAR_INTERACTION_CHECKPOINT_CAPTURE_READY",
        "No gameplay mechanic change",
        "No VISUAL_RUNTIME_PASS claim",
    )
    require(
        "tools/lgo_playable_closure_check.sh",
        "near_interaction_checkpoint_capture",
        "validate_lgo_near_interaction_checkpoint_capture.py",
    )
    require(
        "docs/execution/NEXT-ACTION.md",
        "LGO-NEAR-INTERACTION-CHECKPOINT-CAPTURE-PASS-v1.0",
        "LGO_NEAR_INTERACTION_CHECKPOINT_CAPTURE_READY",
    )
    require(
        "docs/execution/TASK-LEDGER.md",
        "LGO-NEAR-INTERACTION-CHECKPOINT-CAPTURE-PASS v1.0",
        "LGO_NEAR_INTERACTION_CHECKPOINT_CAPTURE_READY",
    )
    check_frozen()
    if ERRORS:
        print("LGO NEAR INTERACTION CHECKPOINT CAPTURE VALIDATION FAILED", file=sys.stderr)
        for error in ERRORS:
            print(" - " + error, file=sys.stderr)
        return 1
    print("LGO_NEAR_INTERACTION_CHECKPOINT_CAPTURE_VALIDATION_PASS")
    return 0


if __name__ == "__main__":
    raise SystemExit(main())
