...
Trạng thái: **OWNER_APPROVED_DESIGN_SET / SHARED_LAYOUT_LOCKED / CHARACTER_LAYOUT_LOCKED / STORAGE_DETAIL_FIX_REQUIRED**
...
Shared layout tổng đã được sửa và khóa lại bằng Player/evidence `build/map01a-five-tab-layout-runtime-v4/{pc,mobile,tablet}/`. Chi tiết tab `Nhân vật` đã qua Player gate tại `build/map01a-character-inspector-runtime-v1/{pc,mobile,tablet}/`: 10 slot có level thật, identity/vitals đầy đủ, item hero không lặp state và footer action giữ đúng composition ở `1600×900`, `1600×720`, `1024×768`. Gate active chuyển sang `Rương đồ`; ba tab sau vẫn chờ đúng thứ tự, không làm song song. Không resume class/pose/wardrobe/source và không rollback code class.
