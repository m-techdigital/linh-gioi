...
## Krita source round-trip phải giữ draw order và alpha authority — 2026-09-13

Tạo đúng tên layer và mở lại được KRA vẫn có thể xuất hình sai. Neutral body v1 dùng thứ tự liên tục tự nghĩ thay cho draw order trong manifest nên tay xa phủ lên ngực; v2 sửa thứ tự nhưng không xóa paint layer mặc định của tài liệu mới, làm composite PNG có alpha 255 toàn canvas. V3 chỉ được nhận sau khi dùng đúng draw order `51..62`, xóa default layer, kiểm từng component vừa có pixel hữu hình vừa có pixel trong suốt, rồi so sánh composite với idle authority: zero pixel và zero alpha drift. Quy tắc tái dùng: importer native phải đọc order từ authority, xóa/kiểm mọi layer mặc định và assert alpha extrema trước khi save/reopen/export; không sửa từng pixel để che lỗi stack hoặc background.

Một garment rest master có giá trị lâu dài khi surface variant dùng chung geometry/topology. Probe upper Pháp giữ đúng một master, A/B là clone và B dùng filter material; alpha hai bản giống tuyệt đối, màu thay đổi trong garment và composite không đổi bên ngoài alpha áo. Gate này thay được việc nhân bản texture chỉ để đổi màu, nhưng chưa thay mesh/weight/deformation proof. Trigger kiểm lại là đổi master texture, filter/material, canvas/source profile hoặc cơ chế clone/export; đổi pose runtime không yêu cầu tạo lại source raster.

Tự động hóa Scripter trên macOS có một lỗi vận hành dễ lặp: gán `AXValue` làm mã hiện trong editor nhưng plugin chưa đánh dấu document đã sửa, nên nút Run không thực thi. Sau khi gán nội dung cần focus đúng text area editor, gửi một newline để phát sự kiện thay đổi rồi mới `AXPress` Run; luôn xác nhận bằng report/output thật, không suy từ việc mã đã hiện trên màn hình.

## Layer compositing không đồng nghĩa với bộ phận skeletal — 2026-09-13

Round-trip KRA v3 có 12 layer không rỗng và ghép lại giống idle authority tuyệt đối, nhưng Player v8 cho thấy nhiều layer là patch/occluder chứa gần cả cơ thể hoặc lỗ khoét, không phải bộ phận có ownership độc quyền. Khi gắn từng layer vào xương, tay bị nhân đôi dù alpha/canvas/hash/draw order đều pass. Vì vậy source contract skeletal phải kiểm nội dung ownership, vùng phủ/chồng và joint cap; tên layer, alpha không rỗng và composite identity chỉ chứng minh authoring stack.

Probe v9 đổi sang mười cutout alpha độc lập nhưng đặt pivot theo tâm bbox, làm khớp vai/khuỷu/gối hở. Probe v10 dùng một công thức chung: lấy trọng tâm alpha của dải 8% ở hai đầu cutout, xoay trục đầu–cuối vào trục xương bind và scale đều theo chiều dài xương đã đăng ký. Cách này bỏ chỉnh offset theo pose/item, giữ root scale `0`, bone-length drift dưới `0,000001` và không lật triangle qua đủ timeline; tỷ lệ jump/attack không đổi theo bbox pose. Công thức được đưa vào runtime helper và regression test để mọi source cùng dùng.

V10 vẫn `FIX_REQUIRED`: source cutout chưa có anchor khớp và vùng overlap/cap có chủ đích nên đường nối còn rời; seam trang phục–body chưa được đo thật; capture chỉ có 12/14 frame Player và thiếu coverage nhãn cho mọi state. Gate v12 đo khoảng cách từ từng joint authority đến alpha của hai part liên quan trong source-space: 6/9 pass ≤2px; fail tập trung ở `torso-hips` tại vai gần 2,35px, hông gần 17,75px và hông xa 14,42px. Không sửa sáu joint đã pass; re-author coverage/cap của torso theo ba socket fail rồi chạy lại đúng gate này. Không chữa bằng nắn pixel hoặc offset từng joint. Bước kế tiếp phải thêm proximal/distal anchor cùng overlap/cap metadata vào source QA, sau đó đo continuity trong Player. Số đo body này là chi phí setup một lần; animation và item sau dùng skeleton/anchor chung. Trang phục mới chỉ author mesh/weight/slot theo contract, và item unseen là phép kiểm xem có thật sự không cần đo lại body hay không.
