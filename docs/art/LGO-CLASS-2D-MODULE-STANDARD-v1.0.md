...
### Khóa nhịp trái/phải cho source sáu pose — 2026-09-14

Bốn frame chạy phải lưu danh tính giải phẫu, không chỉ silhouette hoặc nhãn gần/xa: `contact_left` có chân trái và tay phải tiến; `passing_to_right` có gối phải và tay trái tiến; `contact_right` có chân phải và tay trái tiến; `passing_to_left` có gối trái và tay phải tiến. Cặp 2/4 và 3/5 phải đối xứng theo danh tính chi, và trong mọi frame tay tiến luôn đối bên với chân/gối tiến. Dùng màu/debug layer cố định cho LEFT/RIGHT khi authoring; bỏ debug color chỉ sau khi depth/overlap vẫn đọc đúng.

Source pose dùng một profile chiều dài xương theo giới và chỉ đổi góc khớp/fold thân. Không suy tỷ lệ từ bbox từng pose, vì `jump_tuck` có bbox thấp hơn dù xương không ngắn đi. Bằng chứng control hiện hành nằm ở external `common-character-v3/pose-control-source-blender-v1`: file Blender mở lại được và audit sáu pose giữ nguyên mọi đoạn tay/chân. Mannequin chỉ là structural control, không phải final art hoặc runtime body tách mảnh.

Đã kiểm và dừng ba đường ImageGen: sinh cả sheet, sinh từng frame bằng stick guide, và chuyển từ Blender render. Cả ba đều có thể giữ hình người nhưng làm mất danh tính trái/phải hoặc trả passing thành contact; ảnh tĩnh đẹp không chứng minh chu kỳ đúng. Vì vậy không tạo tiếp nữ/outfit từ output này và không sửa prompt lần nữa. Transfer hợp lệ cần artist-controlled whole-body redraw/paint-over hoặc workflow có cả pose/depth control lẫn identity conditioning; gate đầu là playback cố định 2→3→4→5 của một mẫu nam.
