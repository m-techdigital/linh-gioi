...
- referenced V2 registry properties: 29, including 13 fallback-only by exact V3B property-name coverage.
...
5. V2 fallback assets: current `Resources` payload is 2389.7 KB across 65 images; 29 V2 registry properties are still referenced and 13 are fallback-only by exact V3B property-name coverage, so retire only after dependency checks prove V3B/newer coverage is complete.
