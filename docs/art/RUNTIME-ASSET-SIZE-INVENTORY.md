...
- referenced V2 registry properties: 0, including 0 fallback-only by exact V3B property-name coverage.
...
5. V2 fallback assets: current `Resources` payload is 2389.7 KB across 65 images; 0 V2 registry properties are still referenced by the runtime scan and 0 are fallback-only by exact V3B property-name coverage, so the remaining V2 `Resources` payload is structural cleanup debt rather than an active fallback dependency.
