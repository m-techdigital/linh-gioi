...
The ten-part cutout and whole-base weighted-mesh fixtures generated through spine-csharp in Unity are `WRONG_TEST_SCOPE_OWNER_REJECTED`. They did not create/save/reopen/export an LGO Spine project and must not be resumed or renamed as proof. The required path starts with the actual full LGO character and Pháp Lv1 equipment in Spine Editor, runs authored idle/run/jump there, saves and reopens the source, exports it, then runs the export in `client/Unity`.
