...
- Cảnh Linh Thành: tháp nền đá rộng/tầng gỗ lùi, cửa song/lan can/giằng bốn mặt; cổng có phù hiệu, phố có mái che/cửa chớp theo board v4. Kit72.392tri/8 material/ba texture nhỏ, không thêm texture theo căn nhà hoặc collider tuyến. Build `city-pagoda-build.log` thành công343.469.059B/0 lỗi/19 cảnh báo; quick `city-pagoda-quick.log`, Player toàn tuyến và wrapper `city-pagoda-mobile-focus.json` exit0. Ảnh street-outlook/forecourt đã xem tại `build/visual-evidence/onboarding-blockout/city-pagoda-mobile/`. Batch trước surfaces giảm129.048→58.184tri, texture ngói/vữa và tán lá rời vẫn giữ; lỗi foreground sau shutdown của capture trước nằm ở `city-surfaces-focus-cleanup.txt`, không sửa log cũ. NEXT: xử lý khoảng trống cảnh nền quanh cổng và nối bố cục phố/sân với skyline theo reference, không thêm hệ thống ngoài scenario; chưa art final/FPS điện thoại.
