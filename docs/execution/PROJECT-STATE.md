...
Checkpoint hành trang tabs: runtime Map01A dùng modal theo reference ưu tiên v2 nhưng đã thích nghi cho 2D hiện tại. `Hành trang` và `Thông tin` là hai tab cấp chính, không gộp. Hành trang mặc định hiển thị chi tiết món bên trái và grid túi bên phải; Thông tin hiển thị chi tiết món bên trái và nhân vật + 10 slot bên phải. Khi mở modal ẩn quest/minimap/combat/action bar để không chồng UI; đóng giữ loadout. Evidence Player mặc định: `build/map01a-inventory-tabs-player/quest-capture/{pc,tablet,mobile}/07-q04-inventory-open.png` đã xem; Q01–Q09 capture đủ 18 frame/profile + 38 thoại + 6 NPC revisit. Build tabs `errors=0 warnings=13` deprecated sẵn, EditMode `build/map01a-inventory-redesign-tabs/editmode.xml` 10/10 pass, Python pose atlas 12/12, registered outfit capture 19/19, no-3D/no-source/frozen diff sạch trước refactor và sẽ chạy lại guard trước checkpoint. Không dùng click hệ điều hành/chuột thật để lấy evidence; tab Thông tin được khóa bằng test nội bộ. Chưa mở rương đồ/kho, login/chọn nhân vật hoặc nghiệm thu wardrobe/class art tổng thể.
