...
- Đông Môn tilemap runtime spine checkpoint: map catalog có `DongMonTileDefinitions` cho ground grass/stone, wood platform, jump gap, dash lane và slime arena; runtime scene render tile strip/platform/gap/dash/slime cues, visual manifest ghi `runtimeTilemapSnapshot`, Player capture 10 frame đã review không che HUD/action chính.

## Next

Tiếp tục roadmap 2D bằng authored tilemap chunks/parallax spacing cho Đông Môn theo scenario production spine hoặc nâng inventory panel sang inspect detail/icon grid đẹp hơn sau khi có art asset sạch. Map work phải bám `docs/02-GDD.md`, `docs/design/LGO-2D-SOCIAL-ACTION-DIRECTION-LOCK-v0.1.md`, `docs/design/LGO-2D-MAP-AZ-DESIGN-v0.1.md` và không mở HP/loot/economy/server-authoritative combat khi chưa có gate riêng.
