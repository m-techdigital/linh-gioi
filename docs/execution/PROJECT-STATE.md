...
2026-09-14 active character goal is `LGO_CHARACTER_BASE_SIX_POSE_REBUILD_01`, phase `OWNER_VISUAL_REVIEW`, status `NEED_HUMAN_VISUAL_REVIEW`. The active method is two whole-pose six-frame bases, male and female, with pose order `idle`, `run_contact`, `run_down_recoil`, `run_passing`, `run_flight`, `jump_tuck`. Owner selected the recent separate Spine-test male/female characters as the visual identity authority. Their exact ImageGen payloads were recovered from Codex thread history after cleanup and preserved with hashes at `/Users/minhdc/Projects/Design/LGO-Selected-2D-Source-v1/class-work-in-progress/common-character-v3/identity-authority-recovered-spine-test-v1/`.

Current male/female boards show canonical detachable hair for honest art-direction review and keep neutral underlayers for later equipment composition. They are RGB review boards, not true-alpha/editable source, and are not runtime eligible. No Player or outfit claim is allowed until owner accepts the paired visual design and the accepted poses are reconstructed as registered, reopenable source.
