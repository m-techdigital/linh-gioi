...
- Cảnh Linh Thành: thay gân mái bằng texture ngói lặp128×128, thêm bề mặt vữa/đá128×128 và tán lá rời thay khối hoa. Kit giảm129.048→58.184tri (~55%), giữ8 material; không suy ra FPS. Build `city-surfaces-build.log` thành công343.466.343B/0 lỗi/19 cảnh báo; quick `city-surfaces-quick.log` qua. Player `city-surfaces-mobile.log` hoàn tất toàn tuyến NPC/đá/sân/camera và shutdown; ảnh arrival/forecourt đã xem tại `build/visual-evidence/onboarding-blockout/city-surfaces-mobile/`. Wrapper foreground exit1 vì AppleScript-609 khi Player vừa đóng, giữ phân loại tại `build/dev-loop/city-surfaces-focus-cleanup.txt`; không claim wrapper PASS. NEXT: macro mặt tiền/tháp và scale phố theo board; tiếp đo scene trên target khi có thiết bị, không mở frozen systems hoặc claim art final.
