...
2026-09-15 active task `LGO_RIGID_OUTFIT_PILOT_01` ở phase `FINAL_OUTFIT1_NATIVE_LAYER_ART_AUTHORING`, status `CONTINUE`. Candidate v9 `build/rigid-outfit-pilot/final-1-coherent-body-design-v9/male-female-coherent-bind-design-v1.png` là thiết kế nguyên khối nam/nữ mới: cùng bind stance, hướng phải gần side 3/4, đầu–cổ liền, không có joint marker hoặc mảnh ghép lộ. Tool `tools/build_lgo_rigid_bind_design_profile.py` đăng ký hai body vào canvas `1024×1536`, CharacterRoot `(512,1484)`, pelvis `x=512` và cùng 22 landmark semantics; tests 4/4 pass. Tool cũng tạo ORA 16 author layer cho mỗi giới tính. Krita GUI/Scripter 5.3.3 đã mở ORA, lưu KRA, đóng/mở lại, kiểm 16/16 layer và xuất preview; report `final-1-coherent-body-design-v9/krita-native-template-roundtrip-v1/roundtrip-report.json` là `KRITA_NATIVE_TEMPLATE_ROUNDTRIP_PASS`. Author layers hiện vẫn rỗng, nên chưa phải body source/runtime.
