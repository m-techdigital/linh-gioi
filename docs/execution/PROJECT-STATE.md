## Hiện hành — Pháp nam Lv1 đủ 10 slot sáu pose và đã qua Player — 2026-09-12
...
Nguồn hiện hành ngoài repo: `class-work-in-progress/phap-lv001/complete-garment-authoring-v1/` trong selected-source root; `authoring-selection.json` chọn đủ 10 slot ở `registered-six-pose-v1`. Tất cả có idle/bốn nhịp chạy/jump trên body bất biến. Áo trong được vẽ tới hem dưới đai; overlap với áo ngoài 26.540–46.268 pixel nguồn tùy pose. Jump sửa biên lưng trên source áo, có edit mask/provenance; không scale body. `head_hair` là slot cấu trúc trong suốt ở Lv1 vì tóc đen thuộc base bất biến; metadata ghi rõ base sở hữu default hiển thị, tier sau có thể thêm headwear. Fallback vẫn chứa đồ Võ, không phải base trung tính.
...
Sáu slot bổ sung `arm_guard`, `footwear`, `lower_body`, `class_accessory`, `main_weapon`, `head_hair` dùng cùng canvas/pivot và profile lớp chung. Pháp khí sau body giữ một identity vòng đồng, lõi cyan và tassel qua đủ sáu pose; áo, quần, đai, giày, giáp vai và phụ kiện tháo trả đúng underlayer thay vì xé full outfit. Mười atlas đã kiểm reconstruction pixel-exact cho component có hình; atlas body và body manifest được giữ byte-identical với Võ authority.

Pack runtime hiện hành `build/phap-source-pose-review-complete-garment-v2/pack`; bản v1 bị loại trước Player vì đã decorate body manifest. Evidence `build/phap-complete-garment-lv1-runtime-v4/pc` có 154 Player frame, `errors=[]`, 20 action transition, 40 toggle, 30 base-pose frame, `maxBodyVariants=1`, `maxEquipmentAttachments=17`, bind-return error tối đa `0,000007629` và root scale luôn 1. Ảnh Player đã được xem ở kích thước gốc: idle và bốn nhịp chạy giữ silhouette người liền; jump co/xoay theo body, không phóng scale; main weapon, outer top, lower body, boots, shoulder guard và accessory đều mất đúng contribution ở đứng và trên không. Lượt capture truyền pack nữ canonical-v2 cũ chỉ để thỏa harness bắt buộc hai giới; không dùng frame nữ làm bằng chứng candidate mới.

Validation: pack/compose/semantic `26/26`, capture `18/18`, no-3D và no-source-images đạt. Trạng thái Pháp nam Lv1 là `AGENT_VISUAL_PASS / REVIEW_ONLY`, chưa phải owner/production approval. Action: author Pháp nữ Lv1 cùng contract, sau đó Lv10 hai giới và mixed; chỉ qua gate Pháp mới thay Kiếm/Cơ/Linh theo cùng chuẩn. Goal vẫn active.
