...
2026-09-15 active task `LGO_RIGID_OUTFIT_PILOT_01` ở phase `FINAL_OUTFIT1_NATIVE_LAYERED_SOURCE_AUTHORING`, status `FIX_REQUIRED`. Board v6 `build/rigid-outfit-pilot/final-1-body-outfit-bind-design-v6/body-only-and-dressed-neutral-pair-v1.png` chỉ là authority về identity/silhouette, không phải source. Composite-mask v7 và ImageGen atlas v7 đã bị bác vì không tạo được hidden geometry/layer ownership có thể lắp và xoay ổn định; evidence giữ dưới `build/rigid-outfit-pilot/rejected-iterations/v7-*`. Bước hợp lệ là vẽ body-first bằng source layer thật, kiểm body-only rotation, rồi mới author outfit trên cùng bind source.

Không dùng ảnh sinh nhiều pose làm authority: ba lượt đã lặp sai lead-leg. Motion authority là bộ sáu pose cũ của owner; candidate neutral chỉ định identity/outfit silhouette. Tiếp tục bằng native body-first authoring, không cắt candidate neutral thành module. Outfit #2, mix, class và level vẫn khóa; mọi evidence side-profile/too-frontal/duplicate-phase giữ dưới `build/rigid-outfit-pilot/rejected-iterations/`.
