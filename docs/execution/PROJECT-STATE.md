...
2026-09-15 active task `LGO_RIGID_OUTFIT_PILOT_01` ở phase `FINAL_OUTFIT1_CANONICAL_BIND_REGISTRATION`, status `CONTINUE`. Candidate v9 `build/rigid-outfit-pilot/final-1-coherent-body-design-v9/male-female-coherent-bind-design-v1.png` là thiết kế nguyên khối nam/nữ mới: cùng bind stance, hướng phải gần side 3/4, đầu–cổ liền, không có joint marker hoặc mảnh ghép lộ. Tool `tools/build_lgo_rigid_bind_design_profile.py` đăng ký hai body vào canvas `1024×1536`, pelvis `x=512`, ground `y=1484` và cùng 22 landmark semantics; review board ở `canonical-bind-profile-v1/male-female-canonical-bind-review-v1.png`. Tests 3/3 pass. Đây vẫn là design candidate RGB, chưa phải source layer/runtime.
