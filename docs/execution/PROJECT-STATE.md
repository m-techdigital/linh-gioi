...
- The initial local fixed-size experiment (`b1bd24ca`) proved the direction but was explicitly superseded before push acceptance because fixed `640` shell height / `4.5` camera would be brittle. Effective authoritative HEAD `404b89636fe76f6659935f2a099fcb9fc22ef95b` replaces that policy with one Foundation-level `RuntimePresentationScaleProfile`.
- Responsive architecture: PanelSettings remains one 1672x941 height-normalized base; Foundation derives presentation density from window class + landscape aspect; `RuntimeUiLayoutProfile` consumes that shared scale; Character Hub preserves canonical 1098x724 aspect while fitting safe window; Entry/Auth existing mobile density is capped by the same profile; Character Select/Menu/Dialogue continue their ratio/clamp profile geometry; gameplay touch targets are not visually compressed; `RuntimeWorldPresentationProfile` uses the same curve with gentler camera scaling.
- Current curve parameters are semantic presentation tokens, not per-device sizes: 16:9 mobile base scale .90, aspect compression exponent .25, floor .82. At 1600x720 / 20:9 the runtime result is `presentationScale=0.85118`; shell 934.59x616.25 panel units / 65.49% screen height; camera 4.11885; actor ratio 0.19801; NPC ratio 0.23065. PC/tablet remain scale 1, shell 1098x724, camera 3.8.
- RED evidence: `uif02r-responsive-profile-red-v2/ui-foundation.xml` 38 total / 35 pass / 3 expected failures (missing shared profile, missing PresentationScale, fixed world camera). GREEN: `uif02r-responsive-profile-green/ui-foundation.xml` 38/38 plus product shell regression 1/1.
- Exact-source final verification on `404b8963`: Unity EditMode 394/394 PASS; capture validator 12/12; shared skin 27/27; style debt 3/3; no-3D/no-source/package/code-governance and fixed-policy scans PASS. Fresh macOS Player build succeeded with errors=0 warnings=44.
- Fresh same-binary Player evidence: Character Hub 44 frames/profile and whole flow 27 frames/profile for PC 1600x900, tablet-simulation 1024x768 and mobile-landscape-simulation 1600x720. Machine audit: `build/ui-fidelity-foundation-v1/uif02r-responsive-audit/responsive-audit.json`; visual boards: `mobile-before-adaptive-after.jpg`, `three-profile-after.jpg`.
- Representative visual review PASS: materially more mobile playfield, canonical Hub shell aspect preserved, no new obvious clipping/overlap/stretching in Character/Bag/Skills/Potential/Spirit Pet or representative arrival/dialogue/combat frames; PC/tablet composition retained.
- Physical mobile remains explicitly unverified because Android/iOS PlaybackEngines/device evidence are unavailable. 44 panel-unit semantic target is preserved; its physical point/dp mapping is deferred to authoritative device gates rather than inferred from macOS pixels.
...
- Audit evidence sau checkpoint phát hiện ba profile v9 đều bị gọi thiếu `-screen-width/-screen-height`, nên cùng ra 1024×768 và không đủ chứng minh responsive. `capture_lgo_character_hub.py` nay là lệnh capture dùng chung cho cả năm tab: Player tự click nội bộ, bắt đúng PC 1280×720, tablet 1024×768, mobile landscape 1600×720, đủ chín frame và manifest; test `4/4`.
- Tự audit v10 so với canonical cho thấy topology đã đúng cấu trúc nhưng còn phẳng. v11 chỉ polish một asset dùng chung: ba lớp rune liên tục, cardinal diamond, quầng node đồng nhất, nền node và lõi thiền liền silhouette/trục mạch; không thêm UI/class branch. PNG 59.678 byte. Full EditMode `287 total / 286 passed / 0 failed / 1 ignored`; Player 0 error/0 warning. Evidence hiện hành: `build/map01a-character-hub-potential-polish-runtime-v11/{pc,mobile,tablet}/{potential-default,potential}.png`; đã xem đủ ba viewport, không cắt/chồng/vỡ topology.
- Trạng thái `NEED_HUMAN_VISUAL_REVIEW`; tiếp tục giữ class art/pose/wardrobe/source và screen khác ngoài scope cho đến khi owner duyệt màn Tiềm năng v11.
...
- Đây vẫn là một shared detail template dựng một lần. Đổi đủ năm class chỉ bind `Summary/Value/CurrentEffect/NextEffect`; reference topology/detail không đổi, Skill và Tiềm năng không trộn component.
- TDD RED `0/1`, GREEN `1/1`; full EditMode `287 total / 286 passed / 0 failed / 1 ignored`; shared validator pass, governance `23/23`, pose pack `12/12`, registered capture `19/19`, no-3D/no-source-images pass.
- Player `build/map01a-character-hub-potential-fidelity-player-v6/LinhGioiOnline.app` build `Succeeded`, `errors=0`, `warnings=48`. Evidence `build/map01a-character-hub-potential-fidelity-runtime-v6/{pc,mobile,tablet}/`, 9 frame/profile, `usesOsMouseOrKeyboard=false`; visual audit ba viewport không thấy wrap/cắt/chồng và detail hierarchy gần canonical hơn v5.
- Trạng thái `NEED_HUMAN_VISUAL_REVIEW`; không mở class art/pose/wardrobe/source hoặc screen khác trước owner review.
...
## Product Auth Foundation — CLOSED current-run revalidation — 2026-09-17

- Design/spec `docs/superpowers/specs/2026-09-17-lgo-product-auth-foundation-design.md`; plan `docs/superpowers/plans/2026-09-17-lgo-product-auth-foundation.md`. Canonical Entry/Login layout was preserved; no redesign was introduced.
- Server foundation is split by ownership: dedicated `auth-credentials-v1.json` BCrypt credential store, `ProductAuthService`, in-memory SHA-256-keyed opaque session registry, and `/auth/login`, `/auth/session`, `/auth/logout`; `/dev/auth/login` remains separate.
- Unity `AccountApiClient` now implements product auth, `ProductAuthSessionState` keeps account/token/expiry in memory only, and Entry routes successful Login to existing Character Select. PlayerPrefs continues to store only the remembered identifier.
- Published implementation chain: `91fc6503`, `e493d555`, `a9f5b9c2`, `3136fb76`, `ef667cd5`, `22845f56`; HEAD/`origin/feature/2d` authoritative at `22845f5611774c430d2be395ec52d063a96de785` before this closure-doc commit.
- Current-run server revalidation on Java 25.0.4.1 + Maven 3.9.16: reactor BUILD SUCCESS; `56 executed / 56 declared / 0 skipped`. Current-run Unity EditMode: `323/323`, failed/skipped 0.
- Current-run source gates: Python capture/shared UI `39/39`; shared-skin, no-source-image, no-3D, package hygiene, `git diff --check`, frozen protocol/GameData/ADR/design-token diff and scoped auth security scan all PASS.
- Current-run macOS Player build succeeded from the authoritative HEAD: `errors=0`, `warnings=0`, output `build/product-auth-foundation-v1/revalidation/player/LinhGioiOnline.app` (168 MB).
- Auth visual evidence `build/product-auth-foundation-v1/runtime-final/`: 5 frames × 3 viewports = 15 PNG, exact 1600×900 / 1024×768 / 1600×720, generated after `22845f56`; direct eye review confirmed default/authenticating/invalid/success states preserve canonical Login composition.
- Security audit found no fixture password/token in auth evidence, no PlayerPrefs use in product auth client/session state, and no server auth logging of bearer/password material.
- Known non-auth debt remains unchanged: Character Select mobile landscape partially obscures the left secondary motto with the stage; handle with Character persistence/create-character, not by reopening Auth Foundation.
- Next independent subproject: Register + three-stage Password Recovery. Register and Recovery Request may reuse current canonical layouts; missing Verify Code/New Password screens require owner review of proposed new design before becoming canonical/runtime.

## Register + Password Recovery 3-step — CLOSED exact-source runtime evidence — 2026-09-17

- Owner-approved authority: Entry/Login family v5→v9 remains the scene/layout source; Verify Code and New Password use the approved owner-derived recovery proposal while runtime chrome reuses the shipping Character Hub surface/panel/gold-action/input helpers. No alternate auth skin or art direction was introduced.
- Server account authority is neutral `players-v3.json`: dev-key indexing stays separate, product accounts never fabricate a dev key, and v1/v2 files are preserved during migration. Registration normalizes email, enforces 8–128 character passwords + accepted terms, stores BCrypt credentials, returns `201`, handles duplicate identifiers generically, and rolls back empty product accounts on credential failure.
- Recovery is Request → Verify Code → New Password with in-memory challenge/reset state: 10-minute challenge/reset expiry, 5 code attempts, 60-second resend cooldown, BCrypt code hash, SHA-256 reset-token hash, one-time reset grant and account-wide session invalidation after reset. HTTP semantics cover `202/200/204`, generic `400/401/429/503`, with no code/token echo.
- SMTP delivery is real but fail-closed: production must configure `LG_API_RECOVERY_SMTP_HOST` and `LG_API_RECOVERY_SMTP_FROM` (plus provider-specific port/username/password/STARTTLS). Missing delivery configuration returns `503`; the runtime never pretends an email was sent.
- Unity `AccountApiClient` implements product registration/recovery, `ProductAccountRecoveryState` is memory-only, Register submits asynchronously with loading/duplicate/error states, and one Recovery overlay tree binds Request/Verify/NewPassword without changing the Entry scene. Secrets are cleared on cancel/success and never written to PlayerPrefs.
- Implementation commits: `ad91f6ff` persistence v3; `6489bb3d` registration; `a7f07679` recovery lifecycle; `8ca12004` HTTP/delivery; `14ee41af` Unity account client/state; `60ee3e90` product UI; `7563c6a6` recovery-stage capture; `6e847158` lifecycle state evidence; `379299b1` governance/source-image cleanup.
- Exact runtime/evidence source HEAD: `379299b1c629566a7f3070b844b30aefe0a38082`. Final Java reactor: `138/138`, failures/errors/skips 0. Final Unity EditMode with graphic device: `329/329`, failures/skips 0. Relevant Python shared/capture tests: `48/48`. Shared UI skin, no-3D, no-source-images, package hygiene, code governance, frozen protocol/GameData/ADR/design-token surfaces and `git diff --check` all PASS.
- Final macOS Player: `build/register-recovery-product-flow-v1/player-closure/LinhGioiOnline.app`, `errors=0`, `warnings=0`. Closure evidence: `build/register-recovery-product-flow-v1/after-closure/`, 36 PNG across PC `1600×900`, tablet `1024×768`, mobile `1600×720`; covers default/validation/loading/conflict, Request default/validation/loading/503, Verify resend + invalid/expired, and New Password default + password-rule states. Evidence SHA-256: `b6f3a3b2ca520204535f6fe8705fa24f3f6c3587dd65457f5a5f5f7452f2bdaa`.
- Direct visual review of closure Player evidence found no crop/stack/ghost-text/core-action regression and preserved the approved Entry composition. Earlier review-only PNG/JPG files were archived under `build/register-recovery-product-flow-v1/review-archive/` and removed from tracked `docs/design/reviews/` because the branch `no-source-images` policy forbids non-manifest image files in source.
- Next P0 subproject: authenticated Character Select persistence/list/create/load using the existing 3-slot account APIs, including the known mobile motto/stage visual debt. After that, a low-conflict Character Hub product-visible slice is wiring `EquippedSkillIndices` into the world hotbar/cast path.

## Five-class character schema + Map01A runtime-entry migration — CLOSED exact-source — 2026-09-17

- Spec/plan authority: `docs/superpowers/specs/2026-09-17-five-class-map01a-runtime-entry-migration-design.md` và `docs/superpowers/plans/2026-09-17-five-class-map01a-runtime-entry-migration.md`.
- Persistence nâng lên `players-v4.json`: giữ nguyên nghĩa legacy `class.sword/class.martial` và raw XYZ/yaw; thêm canonical runtime class `vo/kiem/phap/co/linh` cùng `runtimeStatesByCharacterId` tách biệt. v3 được migrate mà không fabricate Map01A state và file v3 cũ không bị sửa.
- Compatibility contract: `class.martial→vo`, `class.sword→kiem`; canonical five-class IDs giữ nguyên. Map01A runtime state khóa `map-01a-cong-dong-lam`, `laneX ∈ [-3.8,44.4]`, `facing ∈ {-1,+1}`. Không có state thì vào `-3.6/+1`, tuyệt đối không reinterpret raw XYZ/yaw.
- Product character API vẫn bearer/account-scoped; list/load trả `runtimeClassId` + optional runtime state. `POST /auth/characters/{characterId}/map01a-state` ẩn cross-account/missing cùng `404` và chỉ persist Map01A state mới. Legacy `/position` không tạo Map01A state.
- Unity client có bearer `SaveMap01AStateAsync`, pure `Map01ACharacterEntryMapper`, và Character Select áp mapper output sau load trước khi đóng overlay. Persisted lane/facing cập nhật Map01A route + facing; semantic class được giữ riêng khỏi renderer authority.
- Renderer product không bị giả mạo: evidence character `Linh` vẫn có `runtimeClassId=linh` nhưng `rendererClassId=vo`. `TwoDSourcePoseReview` vẫn `REVIEW_ONLY/runtimeEligible=false` và không được dùng thay production five-class renderer.
- Implementation chain: `836a7635` → `6c0dccb6` → `a11fc644` → `b1c9cad4` → `3b483e59` → `105f5fe6`. Exact runtime/evidence source HEAD: `105f5fe672aa95ecd5c1a5519cfa6711ad743708`.
- Final gates trên exact runtime HEAD: server `79/79`; Unity full EditMode graphics `349/349`; focused runtime `68/68`; legacy M3B `15/15`; capture Python `10/10`; shared-UI `26/26`; shared-skin/no-3D/no-source-images/package hygiene/code governance/frozen-surface audit/`git diff --check` đều PASS.
- macOS Player exact HEAD: `build/five-class-map01a-runtime-entry-v1/player-exact/LinhGioiOnline.app`, build `Succeeded`, `errors=0`, `warnings=0`; binary SHA-256 `fa5e44d71deba21a8337137e7dd6be53ea30b4c5331b8ce68d95198048eede73`.
- Player evidence: `build/five-class-map01a-runtime-entry-v1/entry-evidence-exact/`; PC `1600×900`, tablet `1024×768`, mobile `1600×720`. Cả ba chứng minh Character Select → world entry với `laneX=18.5`, `facing=-1`, `routeNode=village-square`, no network và no OS input. Board local: `character-entry-three-viewport-board.jpg`.
- Không PNG/JPG review/rejected/intermediate nào được add Git; mọi visual evidence nằm dưới ignored `build/`.
- Hai gate còn tách riêng: (1) production five-class renderer authority cho Kiếm/Pháp/Cơ/Linh bằng runtime-eligible approved packs; (2) approved live-save lifecycle/autosave trigger trước khi gọi `SaveMap01AStateAsync` trong gameplay thật. Create Character vẫn `DESIGN_GATED` và ngoài scope slice này.

## UI Fidelity Foundation UIF-00..02 — CLOSED current-source evidence — 2026-09-18

- Owner-approved spec/plan: `docs/superpowers/specs/2026-09-17-lgo-ui-fidelity-foundation-design.md` and `docs/superpowers/plans/2026-09-17-lgo-ui-fidelity-foundation.md`. Pre-closure-doc source/evidence HEAD: `5e8a06919f3a289910a75e90f1e9d6881a15d35b`.
- Implementation chain: `967060de` authority matrix → `d43e9019` runtime token authority → `bda5870d` shared product USS primitives → `b00e8034` bounded style debt/split skin ownership → `bc73fba6` singular PanelSettings policy → `dd135d14` viewport/safe-area profile authority → `ab2bdcd3` truthful multi-profile metrics/evidence → `5e8a0691` shared-skin gate made split-partial aware.
- Authority is machine-checkable for 17 product surfaces. `design-tokens.json` is the semantic product-UI palette/touch-target authority; runtime consumes one generated ThemeTokens resource. One shared product stylesheet is attached at the root and role classes own stable primitive presentation. Product semantic palette declarations are now `0`.
- Style-debt gate currently passes at `directStyleAssignments=2236`, `numericStyleAssignments=1263`, `semanticPaletteDeclarations=0`, `skinPartialMaxLoc=843`, `skinPartialMaxMethods=59`; the former 2,071-line Skin owner is split into bounded shared/Auth/CharacterHub/World partials.
- Product PanelSettings is singular: `ScaleWithScreenSize`, reference `1672x941`, `MatchWidthOrHeight`, `match=1` (landscape-height authority). HUD attach no longer overrides it. `RuntimeViewportMetrics` + `RuntimeUiLayoutProfile` own safe-area/profile decisions; HUD density consumes semantic profile values instead of a second breakpoint table.
- Fresh macOS Player build: `build/ui-fidelity-foundation-v1/player/LinhGioiOnline.app`, `Succeeded`, `errors=0`, `warnings=46`. Character Hub capture lives at `build/ui-fidelity-foundation-v1/character-hub/`. Measured shell is `1098x724` panel units and `0.7694` of panel/screen height under the height-match contract on all three profiles. PC `1600x900` is `macos-player`; tablet `1024x768` and mobile landscape `1600x720` are explicitly `macos-aspect-simulation`, never physical-device proof.
- Fresh closure gates: full Unity EditMode with graphics device `361/361`, failures/skips `0`; authority validator tests `4/4` + matrix `17` surfaces PASS; shared-skin tests `27/27` + validator PASS; style-debt tests `3/3` + budget PASS; capture tests `11/11`; no-3D, no-source-images, package hygiene, code governance and `git diff --check` all PASS. The prior headless-only wrapped-value failure was environment-specific; the exact graphics-dependent test passed `1/1` with Metal on Apple M2 Pro.
- Visual review in this foundation phase is regression-only: representative Character Hub character/skills/bag frames remain centered without observed clipping after the scale change. This does **not** claim UIF-03 canonical screen fidelity.
- Remaining blocker is explicit: final Android/iOS physical sizing/touch proof is still unclosed because this evidence was produced by macOS Player/aspect simulation. Do not convert that limitation into PASS.
- Next scoped program: `LGO-UIF-03 Entry/Auth family canonical fidelity`, starting with Entry/Login owner canonical before Server Select/Register/Recovery; preserve product auth semantics and Create Character remains DESIGN-GATED.

## UI Fidelity UIF-03 — Entry/Auth family canonical fidelity CLOSED — 2026-09-18

- UIF-03 closed sequentially without reopening product-auth semantics: Entry/Login `2d553a46` → Server Select `85028f37` → Register `5842a502` → Password Recovery Request/Verify/New Password `8170a91e`.
- Authority remains machine-bound: Entry v5 canonical, Server Select v7 canonical, Register v8 canonical, Recovery Request v9 canonical-candidate, and owner-approved Verify/New Password proposal v3. The recovery slice reuses one shared auth-flow header/panel recipe; screen code only binds stage content/state.
- Recovery fidelity now includes the missing canonical hierarchy: ornamental subtitle row, verify masked-email/rule/resend footer, two-field new-password reveal layout, password rule, primary gold CTA and quiet back action. Closed account/recovery API behavior was not changed.
- Fresh exact-source verification before recovery commit: Password Recovery hierarchy `1/1`, ProductAccount UI flow `4/4`, full Unity EditMode graphics `370/370` with `0` failed/skipped, product-flow Python `10/10`, shared-skin Python `27/27`, style-debt/shared-skin/no-3D/no-source-images/package-hygiene/code-governance/`git diff --check` all PASS.
- Exact-WIP macOS Player build: `build/ui-fidelity-foundation-v1/uif03-recovery-player-wip-v1/LinhGioiOnline.app`, `Succeeded`, `errors=0`, `warnings=46`. Fresh recovery evidence is under `build/ui-fidelity-foundation-v1/uif03-recovery-wip-v2/`: Request/Verify/New Password on PC `1600x900`, tablet `1024x768`, mobile-landscape `1600x720`, with technical manifests and no OS mouse/keyboard input.
- Direct visual review found no clipping/overlap in the three recovery stages across all three profiles. Verify shows the approved validity/attempt rule plus resend row; New Password shows two reveal fields plus the `8..128` rule; Request retains the approved single-email/CTA/back hierarchy.
- Evidence authority remains truthful: tablet/mobile are macOS aspect simulations, not Android/iOS physical-device proof. The physical sizing/touch gate recorded by UIF-02 remains open and is not converted into PASS by UIF-03 closure.
- Next scoped program is `LGO-UIF-04 Character Select v6 canonical fidelity on live account binding`. Preserve bearer/account isolation and current five-class Map01A migration. Create Character remains `DESIGN_GATED`.

## UI Fidelity UIF-04 — Character Select v6 canonical fidelity CLOSED — 2026-09-18

- Owner authority: `redesign-v6-character-select/01-character-select-CANONICAL.png`; product behavior authority remains the closed bearer/account-bound three-slot Character Select. Source implementation commit: `0aaac30fc503a93e5bb7cd7c44ece0cbe0904e3c`, verified equal to `origin/feature/2d` before this closure-doc commit.
- Presentation was realigned without reopening list/load semantics: the duplicate rectangular stage backdrop was removed so the full scene remains visible; hero staging was enlarged/recentered; the right account panel was narrowed/shortened; selected/empty slot hierarchy, compact detail block, Edit/Delete secondary actions and dominant gold `Vào game` CTA were retained as one right-side flow; the server strip now sits independently near the bottom and Back remains bottom-left.
- Shared ownership was tightened rather than expanding style debt: Character Select layout roles live in shared USS classes and the skin helpers were moved into dedicated `CongDongLamArrivalHud.Skin.CharacterSelect.cs`. Style budget remains capped at `skinPartialMaxMethods=59`; final measured debt is `directStyleAssignments=2152`, `numericStyleAssignments=1178`, `semanticPaletteDeclarations=0`, `partials=15`.
- TDD/evidence: the new v6 hierarchy contract first failed `0/1` on the missing shared class, then passed `1/1`. Closed account binding remained green `8/8`; the legacy three-slot/no-fake-profile contract remained green `1/1`. Final full Unity EditMode on the exact source passed `371/371`, failed/skipped `0`.
- Final source gates passed: whole-flow capture tests `10/10`, shared-skin tests `27/27`, style-debt tests `3/3`, shared-skin/style-debt validators, no-3D, no-source-images, package hygiene, code governance and `git diff --check`.
- Exact-source macOS Player: `build/ui-fidelity-foundation-v1/uif04-character-select-player-final/LinhGioiOnline.app`, build `Succeeded`, `errors=0`, `warnings=44`. Final captures: `build/ui-fidelity-foundation-v1/uif04-character-select-final/` with PC `1600×900`, tablet `1024×768`, mobile-landscape `1600×720`; all three manifests report `map01a-character-select`, `usesOsMouseOrKeyboard=false`, and technical PASS pending visual review.
- Direct visual review and `character-select-owner-before-after.jpg` confirm the intended hierarchy change across all three profiles with no observed clipping/overlap. The capture hero still uses the existing production Võ renderer authority even when account data is Kiếm; no fake non-Võ art was introduced. Create Character remains `DESIGN_GATED`. Tablet/mobile remain macOS aspect simulations, not physical-device proof.
- Next scoped UI fidelity task: `LGO-UIF-05 Character Hub five-tab fidelity and component consolidation`, preserving the closed V59 behavior/artwork unless a concrete defect is proven.


## UI Fidelity UIF-05 — Character Hub five-tab fidelity CLOSED — 2026-09-18

- Runtime/source authority: `3fd986ef7226b552aad9ed441c884a8ffbe37844` (`refactor(ui): consolidate character hub layout ownership`). The commit contains only the intended Character Hub/Inventory shared-layout ownership, Bag title fidelity, narrow EditMode coverage and shared-skin governance changes; incidental Unity map `.meta` plus Graphics/Quality settings drift was excluded.
- Shared layout ownership now has one main-tab role, one inspector-column recipe reused by Inventory/Hub preview, and one locked action-row recipe reused by Skills/Potential/Spirit Pet. Approved five-tab v4 composition remains intact; no auth, persistence, Character Select account binding, Map01A world-entry, renderer, Create Character or GameData behavior changed.
- Player-visible fidelity fix: Bag uses approved title `RƯƠNG ĐỒ` instead of legacy `HÀNH TRANG`. No fabricated stats, progression, pet/item behavior or unapproved art was introduced.
- Exact-source heavy verification: Unity EditMode `372/372`, failed/skipped `0`; macOS Player build `Succeeded`, `errors=0`, `warnings=44`; Character Hub capture `44` frames/profile at PC `1600×900`, tablet `1024×768`, mobile-landscape `1600×720` (`132` total).
- Visual authority audit: `build/ui-fidelity-foundation-v1/uif05-hub-closure-v1/visual-audit.json` = `UIF05_VISUAL_AUDIT_PASS`; owner/before/after boards show no panel clipping, stack overlap, cropped primary action or selected-tab hierarchy regression across Character, Bag, Skills, Potential and Spirit Pet. Tablet/mobile are macOS aspect simulations only.
- Exact-tree proof: staged verified tree `a41c8d3672201ffc7441a73c7805846b3beb340b` equals commit `3fd986ef` tree. Final exact-source gates pass: Character Hub capture `11/11`, whole-flow capture `10/10`, shared-skin `27/27`, style-debt `3/3`; style debt `2141` direct / `1169` numeric / semantic palette `0`; shared-skin/no-3D/no-source-images/package hygiene/code governance/`git diff --check` all PASS.
- Next program candidate is `LGO-UIF-06 Gameplay HUD fidelity`, currently separately proposed/scoped. Do not start it from UIF-05 closure authority alone.
