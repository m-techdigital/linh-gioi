...
Đã thêm flag `--lgo-map01a-entry-capture` để lấy ảnh entry/login trực tiếp trong Player, không dùng osascript, click chuột hoặc bàn phím hệ điều hành. Test khóa: flag này khởi động Map01A nhưng không đi vào quest capture clock nên entry overlay không bị ẩn; entry mở thì safe HUD/marker bị ẩn để tránh lộ UI chơi phía sau modal. Evidence mới: `build/map01a-entry-capture-runtime-v2/entry-login.png`, manifest `usesOsMouseOrKeyboard=false`, 1600×900, status `TECHNICAL_PASS_VISUAL_REVIEW_REQUIRED`. Player build `client/Unity/build/map01a-entry-capture-player-v2/LinhGioiOnline.app` đạt `errors=0 warnings=13`. Đây là evidence kỹ thuật/visual nội bộ, chưa phải owner approval toàn bộ login UI.
