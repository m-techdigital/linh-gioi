...
Pháp nam Lv1 giữ nguyên checkpoint `f23af9e0`: 10 slot × sáu pose trên `vo_male_v3`, pack `build/phap-source-pose-review-complete-garment-v2/pack`, evidence Player `build/phap-complete-garment-lv1-runtime-v4/pc`. Source được chọn duy nhất ở `class-work-in-progress/phap-lv001/complete-garment-authoring-v1/authoring-selection.json` trong selected-source root; source cũ ngoài selection chỉ dùng truy lỗi.

Pháp nữ Lv1 dùng body bất biến `common-female-v1/source-pose-authoring-v1/registered-body-v1` và nguồn được chọn duy nhất `class-work-in-progress/phap-lv001/female-complete-garment-authoring-v1/authoring-selection.json`. Selection đủ `inner_top`, `outer_top`, `lower_body`, `waist_belt`, `arm_guard`, `footwear`, `shoulder_chest_guard`, `class_accessory`, `head_hair`, `main_weapon`; tất cả nằm trên canvas `1024×1536`, pivot đứng/chạy `(512,1484)`, pivot lộn `(512,820)`. Các donor cũ dính mặt, tóc, da hoặc thân đã bị loại khỏi selection. Sheet mới `_source-assets/lv1-independent-equipment-sheet-v1.png` chỉ cấp bốn món độc lập theo equipment grid gốc: head ornament, boot, shoulder guard và talisman; không thay body hoặc hai lớp áo đã review.

Pack nữ hiện hành `build/phap-female-source-pose-review-complete-garment-v1/pack`, overlay divisor 2 và body divisor 4. Preflight `build/phap-female-complete-garment-preflight-v1` nạp đủ 10 item Lv1. Evidence hai giới `build/phap-complete-garment-lv1-both-runtime-v1/pc` có 154 Player frame, `errors=[]`, 20 action transition, 40 toggle, 30 base-pose frame, `maxBodyVariants=1`, `maxEquipmentAttachments=17`, bind-return error tối đa `0,000007629`, pack ổn định trước/sau capture và đủ `idle/run_contact_a/run_a/run_contact_b/run_b/jump_tuck`.
