...
2026-09-15 active task `LGO_RIGID_OUTFIT_PILOT_01` ở phase `FINAL_OUTFIT1_BODY_AND_OUTFIT_BIND_DESIGN_REVIEW`, status `NEED_HUMAN_VISUAL_REVIEW`. Board v6 `build/rigid-outfit-pilot/final-1-body-outfit-bind-design-v6/body-only-and-dressed-neutral-pair-v1.png` cho thấy male/female body-only và dressed cùng neutral bind để review identity, tỷ lệ, underlayer và outfit silhouette. Đây là RGB composite review-only, chưa phải source. Topology v2 có 16 body part + 25 outfit part; mỗi joint có một pivot chung nhưng body cap và outfit cover độc lập. Tháo toàn bộ đồ, body vẫn phải kín/tự nhiên trên toàn safe ROM.
