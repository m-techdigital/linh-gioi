...
Source admission against the real LGO trees also failed before any LGO Spine runtime test. `build/lgo-spine-production-proof-01/lgo-source-admission.json` records `spineLgoRuntimeTestExecuted=false`: active Pháp Lv1 has zero accepted garment art files; the male KRA is technically reopenable with 12 layers but derives from the owner-rejected cutout body and is runtime-ineligible; female has six review PNGs without an editable layered source; current Unity Pháp atlases have true alpha but are `DRAFT_RUNTIME_FIT`, `runtimeEligibleCount=0` paper-doll assets. Do not use any of these as a disguised Spine proof. The next integration run requires both a Professional 4.3.x seat and an owner-accepted reopenable layered male/female + sleeved Pháp Lv1 source package.
