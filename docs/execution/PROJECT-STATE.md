...
The active character/outfit path for this sandbox is now `docs/art/LGO-SIX-POSE-REGISTERED-OUTFIT-PIPELINE-LOCK-v1.md` with execution plan `docs/superpowers/plans/2026-09-13-six-pose-registered-outfit-pipeline.md`: use the existing six-pose body/motion authority and develop registered outfit pieces across pose and level combinations from design/source evidence. Source-layer coverage for the current Pháp Lv1 repair batch is now complete, but visual acceptance is not: `outer_top`, `waist_belt` and `shoulder_chest_guard` all have 24/24 staged A/B front/back files, while `source-board-v4-after-all-slot-staging/visual-findings-v1.json` remains `SOURCE_VISUAL_FIX_REQUIRED_LAYER_COVERAGE_COMPLETE`. Next work is outer_top visual/source-paint polish and regenerated source/mixed boards before any Player pack. Do not rerun the rejected skeletal Player probe or fit garments on that source.
