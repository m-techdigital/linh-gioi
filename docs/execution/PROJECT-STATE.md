...
2026-09-14 active character goal is `LGO_CHARACTER_BASE_SIX_POSE_REBUILD_01`, phase `POSE_CONTROL_READY_ART_TRANSFER_BLOCKED`, status `NEED_OWNER_DECISION`. The method remains two whole-pose six-frame bases with canonical order `idle`, `run_contact_a`, `run_a`, `run_contact_b`, `run_b`, `jump_tuck`. Owner-selected male/female identity references remain at external `common-character-v3/identity-authority-recovered-spine-test-v1`; its former review boards are rejected because run frames 2–5 did not alternate absolute left/right limb phases.

The new external `common-character-v3/pose-control-source-blender-v1` is a reopenable Blender 4.5.5 source for all six poses. It fixes upper-arm, forearm, thigh and shin lengths across every frame and assigns anatomical left/right explicitly. The control loop is structurally ready and reusable across gender/outfit work, but it is not final character art or runtime source.

ImageGen transfer failed at the capability boundary after whole-sheet, per-frame stick and per-frame Blender controls: the outputs collapsed contact A/B into the same limb layer or ignored passing poses. All failed images are dated rejected evidence and the active review directory contains no candidate. ComfyUI/model weights are not installed; ControlNet pose alone would not preserve the selected identity. The next gate is one artist-controlled redraw/paint-over pilot or a complete verified pose-plus-identity conditioning workflow, followed by fixed-canvas playback. No Player/outfit promotion is allowed before that visual gate.

The earlier Spine proof is historical. Official Spine Trial 4.3.26 opened the Mix-and-Match source, and the official 4.3 runtime/sample ran in an isolated Unity project; this proved only local compatibility with the official sample. Attaching two LGO textures was only `API_ATTACHMENT_COMPATIBILITY_ONLY`. The owner rejected programmatic cutout/weighted-mesh fixtures and superseded this as the active route.

The ten-part cutout and whole-base weighted-mesh fixtures generated through spine-csharp in Unity are `WRONG_TEST_SCOPE_OWNER_REJECTED`. They did not create/save/reopen/export an LGO Spine project. Their code, generated pixels, isolated Unity project/cache, Player builds, images, logs and preferences were deleted on owner request: 6,024 files / 1,142,260,581 bytes. Only written lessons, small tombstones and `build/lgo-spine-production-proof-01/wrong-scope-cleanup-2026-09-14.json` remain. Do not treat the old Spine authoring sequence as the current next action.

Source admission against the real LGO trees also failed before any valid LGO Spine Editor test. `build/lgo-spine-production-proof-01/lgo-source-admission.json` records `spineLgoRuntimeTestExecuted=false`: the old male KRA derives from the rejected cutout body, the old female set lacks editable layered source and current Unity Pháp atlases are runtime-ineligible paper-doll drafts. Two owner-authorized source creation attempts were then audited and stopped: v1 is `VISUAL_REJECTED_MANNEQUIN_NOT_LGO`; v2 is `VISUAL_REJECTED_STRUCTURAL_MASK_LOSS`. They remain `DO-NOT-PACK`; this paragraph is historical evidence, not a resume instruction.

Historical/superseded 2026-09-14 character-base correction: Blender generated 12 distinct proportional frames but failed final style/silhouette review. ImageGen then failed twice as a source-preserving route: the whole sheets repeated A/B limb phases and the direct old-base `male/idle` edit changed face, stance and anatomy. All outputs remain quarantined under external `rejected-evidence/2026-09-14`; this is evidence only and no longer a next-action route.
...
2026-09-14 body-source correction: hai bộ sáu pose nam/nữ mới giữ nguyên năm pose motion cũ và chỉ thu `jump_tuck` bằng một affine scale chung `0.948` quanh `(512,820)`. Phép đo mặt độc lập cho ra `0.951` nam và `0.945` nữ; fixed-canvas review không còn jump phóng lớn. Source RGBA 1024x1536 nằm tại external `common-character-v2/six-pose-base-v2-scale-corrected/`, chưa được tự động đưa vào Player. Sheet ImageGen thay toàn bộ pose bị reject vì pose gần trùng và đã quarantine; không tái sử dụng.
...
Rejected-source recurrence was fixed at the source and pack boundaries. Five Pháp trees with final `REJECTED`/`WITHDRAWN` status (Lv1/Lv10, male/female where applicable) were moved out of `class-work-in-progress` into external `rejected-evidence/2026-09-13`; 1,477 files / 537,683,359 bytes are preserved with per-file SHA-256 in `quarantine-manifest.json`. Old paths now contain tombstones only. Source staging audit reads `authoring-selection.json`, and the atlas pack entrypoint rejects quarantine paths, rejected/withdrawn ancestors, PNGs without explicit alpha, and fully opaque PNGs. This preserves lessons without allowing rejected pixels to return under a new name.
...
Resume guard added after owner feedback: `docs/execution/STOPPED-PATHS-AND-RESUME-GUARDS.md` is now the compact lock for paths that must not restart by accident. It stops the rejected skeletal generated-cutout Player branch, flat 2D pattern/panel direct-fit as a production route, and per-pixel garment nudging loops. Future work must either follow the six-pose registered outfit plan below or open a named proof gate with its own source/evidence requirements.

The active character/outfit path for this sandbox is now `docs/art/LGO-SIX-POSE-REGISTERED-OUTFIT-PIPELINE-LOCK-v1.md` with execution plan `docs/superpowers/plans/2026-09-13-six-pose-registered-outfit-pipeline.md`: use the existing six-pose body/motion authority and develop registered outfit pieces across pose and level combinations from design/source evidence. The owner-selected route is `SLEEVED_PHAP_LV1` with whole-body pose images plus registered front/back overlays. Source-layer counts in the old repair batch are complete, but visual/source acceptance is not; its sleeveless run/jump pixels do not satisfy the route. Final item count remains 0. Current contract declaration is valid while `sourceArtifactValidation.status=NOT_VALIDATED`, so no pack or Player promotion is allowed until a native layered sleeved source passes visual review.

Six-pose repair batch evidence: `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/repair-batch-plan.json`, status `SOURCE_REPAIR_BATCH_READY`, `runtimePromotionAllowed=false`, payload hash `718db23e15c361781f7e828f802b5a232f8f14b2c97a3437cabd9b26d50970a3`. Source PNG inventory evidence `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/current-input-image-inventory.json` confirms existing preview/reference PNGs must not be packed directly: several are full-alpha composites, RGB review files, or non-canvas crop boards rather than clean registered layer source. The initial repair-layer gate `repair-layer-audit-v1.json` was `SOURCE_REPAIR_LAYER_GAP`; the current gate `repair-layer-audit-v7-after-all-slot-staging.json` is `SOURCE_REPAIR_LAYER_READY_FOR_VISUAL_BOARD`, failureCount 0.

Partial source staging 2026-09-13 used only clean measured material exports and did not synthesize missing poses. That historical stage had `outer_top` 4/24, `waist_belt` 20/24 and `shoulder_chest_guard` 4/24. Current staged counts after measured repairs are `outer_top=24/24`, `waist_belt=24/24`, `shoulder_chest_guard=24/24`; external repair directories still have `DO-NOT-PACK.md`, and the batch remains review-only until visual source fixes pass.
...
Non-packable authoring workspace: `tools/prepare_lgo_six_pose_source_authoring_workspace.py` generated `/Users/minhdc/Projects/Design/LGO-Selected-2D-Source-v1/class-work-in-progress/phap-lv001/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/source-authoring-workspace-v1` plus build manifest/README mirror under `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/source-authoring-workspace-v1/`. It packages body references, guide overlays, blank templates and destination export paths for all 11 targets while preserving `DO-NOT-PACK`. Follow-up audit `repair-layer-audit-v3-after-workspace.json` is still `SOURCE_REPAIR_LAYER_GAP` with 44 failures, proving no packable source layer was faked.
