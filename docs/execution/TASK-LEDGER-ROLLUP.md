...
`EDITOR_AUTHORING_NOT_STARTED / BLOCKED_SPINE_TOOLING`. Official Trial/runtime Mix-and-Match passed only as a reference. The belt/weapon run is downgraded to `API_ATTACHMENT_COMPATIBILITY_ONLY`. Owner rejected the subsequent programmatic ten-part cutout and whole-base weighted-mesh Unity fixtures as `WRONG_TEST_SCOPE_OWNER_REJECTED`: none created, saved, reopened or exported an LGO project through Spine Editor. Evidence remains quarantined under `build/lgo-spine-production-proof-01/real-character-motion/`. Resume requires Spine Professional 4.3.x plus accepted layered male/female and sleeved Pháp source; the valid sequence begins with full authoring and motion review in Spine Editor, then official export and Unity Player review.
