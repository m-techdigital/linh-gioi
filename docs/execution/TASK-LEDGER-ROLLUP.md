...
Đường active của sandbox là `docs/art/LGO-SIX-POSE-REGISTERED-OUTFIT-PIPELINE-LOCK-v1.md`: dùng sáu pose body/motion hiện có làm authority tạm thời, hoàn thiện Pháp Lv1 registered outfit theo source/design, kiểm đủ `inner_top`, `outer_top`, `waist_belt`, `shoulder_chest_guard`, rigid accessory, A/B hoặc level variants và mixed-loadout board trước Player pack. Coverage hiện có: `inner_top` và `class_accessory` đủ sáu pose A/B; `outer_top`, `waist_belt`, `shoulder_chest_guard` và jump_tuck còn blocking source/design gaps. Partial staging đã đưa clean measured exports vào repair dirs: `outer_top` 4/24, `waist_belt` 20/24, `shoulder_chest_guard` 4/24; source board đã được xem và ghi `SOURCE_VISUAL_FIX_REQUIRED`. Repair-layer audit sau staging còn 44 missing layer failures. Missing-source brief gom thành 11 slot/pose target có anchor source-space từ guide sạch outlier; guide board đã được xem và ghi `AUTHORING_GUIDE_BOARD_VISUAL_CHECKED` sau fix nền opaque. Selection audit `source-staging-selection-audit-v2.json` sạch cho nguồn đang stage nhưng ghi rõ không reuse `outer-top-run-four-material-v1/v2` vì bị `priorRejected`, và không dùng v3 bodymask. Next: author source cho outer_top run+jump, guard run+jump, waist jump; không synthesize jump từ idle/run, không xét pack Player. Không claim technical pass là visual acceptance.
