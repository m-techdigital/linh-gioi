...
Đường active của sandbox là `docs/art/LGO-SIX-POSE-REGISTERED-OUTFIT-PIPELINE-LOCK-v1.md`: dùng sáu pose body/motion hiện có làm authority tạm thời, hoàn thiện Pháp Lv1 registered outfit theo source/design, kiểm đủ `inner_top`, `outer_top`, `waist_belt`, `shoulder_chest_guard`, rigid accessory, A/B hoặc level variants và mixed/off-slot board trước Player pack. Source-layer coverage của repair batch hiện đã đủ: `repair-layer-audit-v7-after-all-slot-staging.json` là `SOURCE_REPAIR_LAYER_READY_FOR_VISUAL_BOARD`, failureCount 0, với `outer_top=24/24`, `waist_belt=24/24`, `shoulder_chest_guard=24/24`; A/B board và 7 mixed/off-slot scenario đều missingLayerCount 0. Visual acceptance vẫn fail: `SOURCE_VISUAL_FIX_REQUIRED_LAYER_COVERAGE_COMPLETE`, blocker chính là `outer_top` run/jump còn đọc như body-painted/sleeveless so với idle áo tay trắng. Next: polish source visual trong repair dirs, regenerate source + mixed/off-slot boards, rồi mới xét review-only Player pack. Không quay lại author 11 target, skeletal/cutout, flat panel production hoặc chỉnh từng pixel không có envelope/evidence.
