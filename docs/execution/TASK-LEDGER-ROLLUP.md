...
Đường active của sandbox là `docs/art/LGO-SIX-POSE-REGISTERED-OUTFIT-PIPELINE-LOCK-v1.md`: dùng sáu pose body/motion hiện có làm authority tạm thời, hoàn thiện Pháp Lv1 registered outfit theo source/design, kiểm đủ `inner_top`, `outer_top`, `waist_belt`, `shoulder_chest_guard`, rigid accessory, A/B hoặc level variants và mixed-loadout board trước Player pack. Coverage hiện có: `inner_top` và `class_accessory` đủ sáu pose A/B; `outer_top`, `waist_belt`, `shoulder_chest_guard` và jump_tuck còn blocking source/design gaps. Repair-batch brief đã chốt từ coverage: `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/repair-batch-plan.json`, status `SOURCE_REPAIR_BATCH_READY`, `runtimePromotionAllowed=false`, hash `718db23e15c361781f7e828f802b5a232f8f14b2c97a3437cabd9b26d50970a3`. Input inventory xác nhận nhiều `preview-on-*` là composite full-alpha hoặc RGB/non-canvas review file, không được pack trực tiếp. Partial staging đã đưa clean measured exports vào repair dirs: `outer_top` 4/24, `waist_belt` 20/24, `shoulder_chest_guard` 4/24; source board `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/source-board-v1/contact-sheet.png` đã được xem và ghi `SOURCE_VISUAL_FIX_REQUIRED`. Repair-layer audit sau staging còn 44 missing layer failures. Next: author source cho outer_top run+jump, guard run+jump, waist jump; không synthesize jump từ idle/run, không xét pack Player. Không claim technical pass là visual acceptance.
