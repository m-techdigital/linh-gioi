...
Use the indexed `LGO-VISUAL-EVIDENCE-PROFILE-INDEX-PASS-v1.0` desktop/tablet/mobile screenshots to tighten login scaling, logo footprint, CTA/panel rhythm, and mobile/tablet layout proportions without importing new heavy art or changing account semantics. Marker already ready: `LGO_VISUAL_EVIDENCE_PROFILE_INDEX_READY`.
...
`LGO-VISUAL-EVIDENCE-PROFILE-INDEX-PASS-v1.0` is evidence-ready. Continue with `LGO-LOGIN-RESPONSIVE-SCALE-CLEANUP-PASS-v1.0`, using profile screenshots for proportional layout decisions.
