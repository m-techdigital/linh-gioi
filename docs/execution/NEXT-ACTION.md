...
2. UIF-07 authoritative runtime source is `b4a5303bb98f8884a3db3b8f7c4d7c7617a41b02`, pushed remote-equal before closure docs: one `RuntimeWorldPresentationProfile` owns profile classification/scale targets, Map01A exports measured actor/NPC/camera/label/coverage metrics, and legacy PlayableWorld reuses the shared classifier without changing its historical smoke values.
3. Exact-source evidence is under `build/ui-fidelity-foundation-v1/uif07-world-final-v2/`, `uif07-world-player-exact-v2-build.log` and `uif07-world-after-v2/`: Unity EditMode 388/388 PASS; macOS Player build Succeeded with errors=0 warnings=0; PC/tablet/mobile quest captures 18 frames/profile PASS; machine audit + owner/AFTER visual review PASS.
4. Measured actor screen-height ratios are 0.214739 PC / 0.214410 tablet / 0.214593 mobile, inside the 0.18–0.27 target band; NPC ratio is 0.25 across profiles inside the 0.16–0.27 band. Camera half-height is 3.8, ground offset 1.5, max foot error 0, parallax delta 32.2, and background coverage scale is positive on all profiles. Tablet/mobile remain macOS aspect/profile simulations, not physical-device proof.
5. Continue sequentially with assigned **LGO-UIF-08 Runtime texture/mobile memory**: inventory current shipping textures/atlases/Resources and make explicit platform decisions from measured memory/quality evidence. Do not blind-compress assets, redesign art, or pull UIF-09 decomposition/UIF-10 device-gate work forward.
