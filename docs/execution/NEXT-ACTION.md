...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, north-star and mobile/tablet UI reference sheets anchor layout decisions, `RuntimeUiOverflowGuard` centralizes bounded action rows, scroll regions, overlay placement, and responsive action columns. World dialogue now has explicit header/body/footer ownership, body-only scrolling, viewport-centered overlay placement with equal top/bottom insets, and a long-text evidence checkpoint. Session menu mobile now centers through the same overlay base, first-time Character Hall hides the invalid enter-world action until a character exists, mobile selected Character Hall uses a compact bottom action dock, and shared button tiers reduce one-off button sizing; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
