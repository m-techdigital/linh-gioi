...
Soften the login Gate Keeper grounding shadow on tablet/desktop so the NPC feels staged in the scene instead of standing on a dark UI smear. Marker: `LGO_LOGIN_CTA_BACKING_EVIDENCE_REFRESH_READY`. Do not add new image payload or change login behavior.
...
`LGO-LOGIN-CTA-BACKING-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-LOGIN-NPC-GROUNDING-SHADOW-BALANCE-PASS-v1.0`; tablet/desktop screenshots show the NPC grounding shadow can be softer and more scene-integrated.
