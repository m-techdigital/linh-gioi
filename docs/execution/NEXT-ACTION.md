...
Audit build/source weight after the visual passes, document PC/tablet/mobile budget targets, identify removable generated/cache/build artifacts, and add a lightweight report/validator for asset and build payload growth. Do not delete dependency-bearing assets without verification and do not change gameplay.
...
Next allowed action: run a build-size budget and cleanup pass after the Character Hall composition pass. The visual evidence harness records explicit review checklist categories for every checkpoint while still refusing to claim visual PASS from capture/build alone.
...
`LGO-CHARACTER-HALL-V3B-COMPOSITION-POLISH-v1.0` is closed. Continue with `LGO-BUILD-SIZE-BUDGET-AND-CLEANUP-PASS-v1.0`.
