...
- Nhóm interaction facing hoàn tất: NPC red62.6°/đá red31.4° -> runtime green, model quay đúng mục tiêu mà không tự dịch chuyển. Input mới hủy stone facing và không quay lại khi nhả; cùng thời lượng pulse1.2s, không khóa điều khiển. `stone-facing-green.log` full route exit0, `interaction-facing-quick.log` qua; ảnh dialogue/complete đã xem. Gom checkpoint hai source preview/world. Next: làm rõ đích thị giác lối ra phố tại0/14 theo mặt bằng/storyboard hiện có; chưa map transition, quest persistence hoặc quảng trường hoàn chỉnh.
