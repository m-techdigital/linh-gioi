...
Audit remaining runtime UI style ownership drift after World HUD row/helper evidence refresh. Marker ready from the completed evidence pass: `LGO_WORLD_HUD_ROW_HELPER_EVIDENCE_REFRESH_READY`. Prefer reusable factory/skin/layout primitives only when they remove real duplication; leave screen state and gameplay semantics in the controller.
