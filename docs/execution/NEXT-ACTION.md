...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, north-star and mobile/tablet UI reference sheets anchor layout decisions, `RuntimeUiOverflowGuard` centralizes bounded action rows/scroll regions, World dialogue text is constrained to a vertical `ScrollView` while action buttons stay visible, mobile selected Character Hall uses a compact bottom action dock, session menu pause actions fit desktop/tablet/mobile, and the world hub no longer lets the target dummy compete with the Gate Keeper objective signal until combat focus is active; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
...
- Next implementation task after this fix: expand/follow the target case matrix in `docs/design/RUNTIME-UI-RESPONSIVE-LAYOUT-HELPER-REVIEW-v1.0.md`, then close the largest visible gap in Login, Character Hall, World HUD, or Session Menu using owner-level layout/profile helpers, bounded containers, and profile evidence, not one-off raw `Screen.width/height` tuning.
