...
- Checkpoint v11 giữ template tĩnh 600×520 và polish chiều sâu trên chính asset chung: ba lớp rune, cardinal diamond, quầng/nền node, lõi thiền và trục mạch; class chỉ bind overlay dùng chung. Import giữ đúng native size bằng `nPOTScale: 0`.
- Player: `build/map01a-character-hub-potential-polish-player-v11/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/map01a-character-hub-potential-polish-runtime-v11/{pc,mobile,tablet}/{potential-default,potential}.png`; lần lượt 1280×720, 1024×768, 1600×720; 9 frame/profile, `usesOsMouseOrKeyboard=false`. Capture bằng `python3.12 tools/capture_lgo_character_hub.py ...`; không nhận evidence nếu ba profile dùng sai kích thước.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v11, tập trung độ sâu vòng rune/lõi thiền, alignment icon/tên/value trên năm node và hierarchy current/next/cost ở detail-right trên đủ ba viewport thật.
...
Evidence:
`build/map01a-character-hub-potential-polish-runtime-v11/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- Player v11 `Succeeded`, `errors=0`, `warnings=0`; capture đúng PC/tablet/mobile; frozen diff/`git diff --check` pass. Runtime asset 59.678 byte và toàn bộ skin khóa native-size import.
