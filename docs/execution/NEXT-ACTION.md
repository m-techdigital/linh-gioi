...
- Checkpoint v15 giữ một topology tĩnh 600×520 và catalog/state bất biến cho đủ năm class; shared inspector đã khóa hierarchy chữ/action và loại chồng lấn cost/action phát hiện ở evidence v13. Không đổi asset/class art.
- Player: `build/character-hub-visual-hierarchy-player-v15/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/character-hub-visual-hierarchy-runtime-v15/{pc,mobile,tablet}/{potential-default,potential}.png`; lần lượt 1280×720, 1600×720, 1024×768; 9 frame/profile, `usesOsMouseOrKeyboard=false`. Capture bằng `python3.12 tools/capture_lgo_character_hub.py ...`; không nhận evidence nếu ba profile dùng sai kích thước.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v15, tập trung độ sâu vòng rune/lõi thiền, alignment icon/tên/value trên năm node và hierarchy current/next/cost/action ở detail-right trên đủ ba viewport thật.
...
Evidence:
`build/character-hub-visual-hierarchy-runtime-v15/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- v13 bị loại sau visual audit vì cost/action chồng nhau. v15 dùng shared inspector spacing/helper, Player `Succeeded`, `errors=0`; 50 warning CS0618 là deprecation hiện hữu. Capture đúng PC/tablet/mobile; full EditMode `286/287` pass, 1 ignored. Runtime asset 59.678 byte và toàn bộ skin khóa native-size import.
