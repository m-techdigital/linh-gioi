...
- Checkpoint v7 dựng sẵn topology 600×520, silhouette ngồi thiền, trường năng lượng, trục năm điểm mạch và detail-right theo hero/current level/current/next/cost; `Võ/Kiếm/Pháp/Cơ/Linh` chỉ bind profile/data/icon/text/state vào cùng object tree.
- Player: `build/map01a-character-hub-potential-fidelity-player-v7/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/map01a-character-hub-potential-fidelity-runtime-v7/{pc,mobile,tablet}/{potential-default,potential}.png`; 9 frame/profile, `usesOsMouseOrKeyboard=false`.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v7, tập trung silhouette/trục năm điểm mạch, tỷ lệ topology/năm node-value-add và hierarchy current/next/cost ở detail-right.
...
Evidence:
`build/map01a-character-hub-potential-fidelity-runtime-v7/{pc,mobile,tablet}/{potential-default,potential}.png`

## Checkpoint detail

- Một `CharacterHubPotentialTopology` sở hữu vòng/đường nối/runic anchors/core figure/trường năng lượng/trục năm điểm mạch/năm khung node/năm ô giá trị/năm ô cộng/glyph; overlay chỉ nhận input và bind icon/tên/value/selection.
...
- Player v7 `Succeeded`, `errors=0`, `warnings=48`; frozen diff/change budget/`git diff --check` pass.
