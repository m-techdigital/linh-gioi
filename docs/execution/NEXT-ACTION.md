...
- Current reason: Bóng Tối now shows a clear `Đã yên / Bóng Tối lùi` world label after Training Stone completion, making the safe-loop resolution easier to read without opening enemy combat; screenshot evidence was reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
...
- Next implementation task after this fix: choose the next compact player-visible slice, likely skill feedback timing, local combat button/result clarity, World Hub empty-space richness, or Character Hall final-art parity; avoid heavy assets and one-off validators unless a real gate needs them.
