...
Review whether the World HUD title plus ornament block should become a reusable header primitive after evidence-state helper cleanup. Marker ready from the completed source refactor: `LGO_RUNTIME_UI_EVIDENCE_STATE_HELPER_READY`. Only extract if it removes repeated layout code without introducing stateful screen components.
