...
```json
{"activeTask":"LGO_RIGID_OUTFIT_PILOT_01","phase":"NATIVE_LAYER_ART_EXTERNAL_CAPABILITY_REQUIRED","status":"BLOCKED","method":"DESIGN_FIRST_NATIVE_LAYERED_RIGID_AUTHORING","sourceProfile":"lgo_character_canvas_1024x1536_v1","requiredMotion":["Idle","Walk","Run","Jump","Attack","Roll"],"motionAuthority":"build/character-base-v3/male-old-base-six-pose-reference.png","currentDesignEvidence":"build/rigid-outfit-pilot/final-1-coherent-body-design-v9/canonical-bind-profile-v1/male-female-canonical-bind-review-v1.png","jointContract":"docs/art/LGO-RIGID-JOINT-AND-LAYER-AUTHORING-CONTRACT-v1.md","jointProfile":"docs/art/data/lgo-rigid-joint-interface-profile-v1.json","rejectedSourceEvidence":["build/rigid-outfit-pilot/rejected-iterations/v7-composite-mask-source","build/rigid-outfit-pilot/rejected-iterations/v7-imagegen-atlas-invalid","build/rigid-outfit-pilot/rejected-iterations/v8-procedural-primitive-body-source","build/rigid-outfit-pilot/rejected-iterations/v9-flat-silhouette-width-inference-invalid","build/rigid-outfit-pilot/rejected-iterations/v9-vector-tracing-quality-loss"],"currentSourceAuthorityValid":false,"bodyBindCalibrated":false,"technicalInvariantStatus":"KRITA_TEMPLATE_ROUNDTRIP_PASS_SOURCE_ART_MISSING","visualStatus":"V9_COHERENT_BIND_READY_AUTHOR_LAYERS_EMPTY","stopCondition":"TRUE_LAYERED_BODY_ART_REQUIRED","bodyModuleCount":16,"outfitModuleCount":25,"redesignOutfit1Allowed":true,"nativeLayerAuthoringAllowed":true,"proceduralPrimitiveBodyAllowed":false,"compositeSlicingAllowed":false,"imagegenAtlasAsSourceAllowed":false,"vectorTraceAsSourceAllowed":false,"silhouetteWidthInferenceAllowed":false,"rigImplementationAllowed":false,"outfit2IntegrationAllowed":false,"classExpansionAllowed":false,"perPoseAssetsAllowed":false,"deformationAllowed":false,"animatedScaleAllowed":false,"canonicalBindProfileEvidence":"build/rigid-outfit-pilot/final-1-coherent-body-design-v9/canonical-bind-profile-v1/canonical-bind-profile-v1.json","sourceTemplateValid":true,"sourceArtValid":false,"kritaRoundtripEvidence":"build/rigid-outfit-pilot/final-1-coherent-body-design-v9/krita-native-template-roundtrip-v1/roundtrip-report.json","sourceBlockerEvidence":"build/rigid-outfit-pilot/rejected-iterations/v9-vector-tracing-quality-loss/rejection-report.json"}
```

## Kết quả batch hiện tại

Candidate v8 dạng SVG primitive đã bị bác sau review trực tiếp dù 3 kiểm tra cấu trúc pass: hình mannequin, joint tròn lộ và identity yếu. Output được chuyển vào `build/rigid-outfit-pilot/rejected-iterations/v8-procedural-primitive-body-source/`; tool sinh primitive đã bị xóa và guard cấm resume.

Candidate v9 đã tạo một thiết kế nguyên khối nam/nữ từ authority v6, không có joint marker, outer outfit hoặc exploded part. `tools/build_lgo_rigid_bind_design_profile.py` chiếu mỗi body vào common canvas `1024×1536`, khóa pelvis `x=512`, ground `y=1484` và 22 landmark semantics dùng chung; bản clean và overlay là hai file riêng. Tests `tools/test_build_lgo_rigid_bind_design_profile.py` pass 3/3. Evidence chính: `build/rigid-outfit-pilot/final-1-coherent-body-design-v9/canonical-bind-profile-v1/male-female-canonical-bind-review-v1.png`. Đây là design/profile candidate, không phải layered source hoặc runtime PASS.

Probe đo joint width từ silhouette phẳng đã dừng sau một lượt: foreground tổng thể tách đúng, nhưng vai/hông/gối không có biên semantic nội bộ nên cross-section ăn sang torso/chân còn lại và tạo số đo giả. Không sửa inset để ép số đẹp. Evidence giữ tại `build/rigid-outfit-pilot/rejected-iterations/v9-flat-silhouette-width-inference-invalid/`; chỉ đo `Wb` sau khi từng body part có alpha riêng trong native source.

Authoring template đã chạy thật qua Krita GUI/Scripter 5.3.3. Mỗi ORA nam/nữ có 16 `AUTHOR - BODY/*` layer, guide và clean reference tách riêng; Krita đã lưu KRA, đóng/mở lại, giữ 16/16 layer và xuất preview 1024×1536. Report `build/rigid-outfit-pilot/final-1-coherent-body-design-v9/krita-native-template-roundtrip-v1/roundtrip-report.json` là `KRITA_NATIVE_TEMPLATE_ROUNDTRIP_PASS`. `authorLayersContainArt=false`, nên đây chỉ loại bỏ blocker format/tool; không được gọi là body source hoàn thành.

Vector re-authoring đã dừng sau đúng hai cấu hình. Cả hai giữ broad silhouette nhưng làm tóc/da bệt, cạnh thô hơn source và vẫn cần clip mask để chia ownership; tăng chi tiết chỉ nâng SVG từ khoảng 2.5 MB lên 4 MB. Evidence `build/rigid-outfit-pilot/rejected-iterations/v9-vector-tracing-quality-loss/rejection-report.json` là `VECTOR_REAUTHORING_REJECTED_VISUAL_QUALITY_LOSS`. Không chạy thêm trace hoặc semantic clip từ RGB.

Owner xác nhận kỹ thuật rigid khả dụng, nhưng bác side-profile v4 và candidate 3/4 đầu tiên vì quay ra trước quá nhiều. Candidate v5.2 giữ hướng chạy phải và chỉ yaw 12–15° ra trước từ profile. Outfit được re-author theo seam-cover: cổ cao, cap vai, mép bracer phủ khuỷu, cuff găng, đai bản rộng đúng pelvis, vạt hông ngắn tách mảnh, guard gối và cuff cổ chân phủ rõ shaft–foot. Static crop review không thấy pivot chính nào thiếu cover; chuyển động chưa được chứng minh. Ảnh sáu pose cũ của owner là motion authority duy nhất.
...
Design board v6 tạo đúng một lượt từ dressed candidate: male body-only, male dressed, female body-only, female dressed cùng hướng phải và ground gần khớp. Review trực tiếp không thấy chi rời hoặc ball-joint lộ ở bind. Board vẫn là RGB composite; body/dressed chưa pixel-identical, không có alpha/layer/hidden cap và chưa chạy motion, nên chỉ được dùng duyệt identity, tỷ lệ, underlayer và silhouette outfit.

## Next result người chơi có thể kiểm chứng

**BLOCKER DUY NHẤT: ART LAYER GỐC.** Designer cần vẽ art và hidden geometry vào 16 `AUTHOR - BODY/*` layer trong hai KRA đã mở lại được, hoặc cung cấp true layered source tương đương v9. Sau đó chạy export RGBA, đo `Wb`, body-only rotation board, outfit #1 và Unity. Không tiếp tục bằng primitive, vector trace, suy width từ silhouette, composite clip hoặc atlas ImageGen.

Lượt composite-mask v7 bị bác vì motion lộ mép chữ nhật, khe tay/chân và xoay theo ground shadow. Atlas ImageGen v7 bị bác vì nền opaque, module lặp/sai và không chứng minh lắp lại một bind source. Evidence giữ tại `build/rigid-outfit-pilot/rejected-iterations/v7-{composite-mask-source,imagegen-atlas-invalid}/`; không đổi tên hoặc tái sử dụng.

Immediate-stop rule: bất kỳ review nào thấy sai camera/body axis, tỷ lệ, đai, joint cover hoặc silhouette thì giữ candidate làm rejected evidence và sửa source/design ngay. Cấm tiếp tục tách module, rig, motion tuning hoặc full capture trên design chưa đạt. Sau source approval chỉ dựng 15 key Player rẻ; lỗi cấu trúc ở key phải quay về module/overlap tương ứng trước khi chạy combo.
