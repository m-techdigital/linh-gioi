...
Audit remaining World HUD row/helper coverage after header block extraction. Marker ready from the completed source refactor: `LGO_WORLD_HUD_HEADER_BLOCK_READY`. Only extract stateless repeated layout primitives; leave gameplay state and runtime text in the controller.
...
`LGO-COMBAT-BUTTON-STATE-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-COMBAT-BUTTON-MOBILE-RESPONSIVE-EVIDENCE-v1.0`; the next useful batch is refreshing mobile/tablet profile evidence for the compact combat action button.
