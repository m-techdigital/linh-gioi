...
- Current reason: selected Character Hall now shows a concise `Sẵn sàng` row under the character list, connecting the chosen profile to the `Vào sân luyện` action and reducing empty left-panel space; screenshot evidence was reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
...
- Next implementation task after this fix: choose the next compact player-visible slice, likely movement/input feel, World Hub interaction clarity, or another screenshot-proven UI/UX issue; avoid heavy assets and one-off validators unless a real gate needs them.
