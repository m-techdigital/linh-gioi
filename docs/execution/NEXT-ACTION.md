...
Input inventory evidence: `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/current-input-image-inventory.json`, generated by `tools/inspect_lgo_source_png_inventory.py --scan-alpha`. It confirms there are reusable reference PNGs, but also confirms why direct pack is invalid: `preview-on-*` files are full-alpha composites, some review/crop files are RGB or non-canvas size, and clean six-pose A/B front/back exports are still missing for the repair slots.

Repair-layer gate evidence: `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/repair-layer-audit-v1.json`, generated by `tools/audit_lgo_six_pose_repair_source_layers.py`, is `SOURCE_REPAIR_LAYER_GAP`: `outer_top`, `waist_belt` and `shoulder_chest_guard` each have `present=0/24` in their fixed repair directories. The next worker must author/export those source layers before source board, mixed-loadout board or Player pack.

Partial staging evidence: `build/pose-matched-layer-authoring-v1/six-pose-source-repair-batch-v1/stage-repair-layers-v1.json`, `repair-layer-audit-v2-after-staging.json`, `staged-repair-image-inventory-v1.json` and `source-board-v1/visual-findings-v1.json`. Clean measured exports were staged only where source already existed: `outer_top` idle 4/24 files, `waist_belt` idle+four run poses 20/24 files, `shoulder_chest_guard` idle 4/24 files. The board status is `SOURCE_VISUAL_FIX_REQUIRED`; the audit still has 44 missing layer failures. External batch and repair directories contain `DO-NOT-PACK.md`.
