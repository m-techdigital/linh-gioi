...
Move the remaining session-menu responsive padding decision into `RuntimeUiLayoutProfile` so the pause/settings shell follows the same reusable profile ownership as lobby, World HUD, and dialogue padding. Marker ready from the completed evidence refresh: `LGO_RUNTIME_UI_RESPONSIVE_PADDING_PROFILE_EVIDENCE_REFRESH_READY`.
...
`LGO-RUNTIME-UI-RESPONSIVE-PADDING-PROFILE-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-RUNTIME-UI-SESSION-MENU-PADDING-PROFILE-AUDIT-v1.0`; the next useful batch is moving the remaining session-menu responsive padding into profile ownership.
