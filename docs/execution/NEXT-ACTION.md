> **Checkpoint Map01A Q01–Q09 — 2026-09-10:** Player thật đã chạy một lượt tuyến Hạ Vân → Đại Cổng → Quan Thủ → Quảng trường → Tổng Phú → Thanh Nhi → Giếng/Cầu → Lão Trần → combat/loot → portal. Runtime hoàn tất đủ 9 quest gồm Q08 tùy chọn; Linh Thảo biến mất sau thu thập, rương chuyển trạng thái sau mở, mục tiêu nhận ba hit và portal đổi sang `Đã mở`. Capture v4 có 24 trạng thái trên từng profile mobile/tablet/PC tại `build/map01a-playable/q01-q09-three-profiles-final/`; manifest cả ba ghi `activeQuestId=COMPLETE`, `completedQuestCount=9`, skill/hit `3/3`, target HP `0`. Ảnh thoại, vật thể, combat và portal đã review trực tiếp.
>
> **Next Map01A batch:** hoàn thiện phần còn thiếu của MAP01A-03 theo đúng source: panel hành trang nhìn thấy được, Bình Máu/Bình Linh Lực có số lượng và thao tác dùng, class item loot có thao tác equip, minimap/route unlock được biểu diễn rõ. Gom UI + state + input + capture vào một lượt rồi test/build/capture ba profile; chưa chuyển sang Võ rig và chưa mở Map01B. Checkpoint hiện tại là quest-flow pass, chưa claim toàn bộ Map01A production hoặc chứng nhận thiết bị vật lý.
