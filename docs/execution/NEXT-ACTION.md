...
- Checkpoint v8 giữ topology/detail v7 và sửa lựa chọn mặc định qua profile data: Võ `Sinh lực`, Kiếm `Nhanh nhẹn`, Pháp `Linh lực`, Cơ `Công`, Linh `Linh lực`; cùng object tree được giữ nguyên khi đổi class.
- Player: `build/map01a-character-hub-potential-class-default-player-v8/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/map01a-character-hub-potential-class-default-runtime-v8/{pc,mobile,tablet}/{potential-default,potential}.png`; 9 frame/profile, `usesOsMouseOrKeyboard=false`.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v8, tập trung silhouette/trục năm điểm mạch, tỷ lệ topology/năm node-value-add và hierarchy current/next/cost ở detail-right.
...
Evidence:
`build/map01a-character-hub-potential-class-default-runtime-v8/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- Player v8 `Succeeded`, `errors=0`, `warnings=48`; frozen diff/change budget/`git diff --check` pass.
