...
Audit remaining direct style use in `M4PlayableClientController` after the RuntimeUiSkin usage guide. Marker: `LGO_RUNTIME_UI_SKIN_USAGE_GUIDE_READY`. Classify each repeated block as layout glue, state-specific color, or a safe role-helper/factory candidate without broad refactors.
