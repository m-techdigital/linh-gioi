...
Polish the in-world HUD/action shell so it feels closer to the V3B login/Character Hall visual language without adding combat mechanics. Focus on panel hierarchy, button balance, compact copy, and desktop/tablet/mobile readability.
...
`LGO-VISUAL-RUNTIME-FAST-PROFILE-REUSE-PASS-v1.0` is source-ready. Continue with in-world HUD/action shell V3B skin polish, preserving current gameplay and contract semantics.
