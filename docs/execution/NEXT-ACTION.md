...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, World labels/prompts use one World viewport profile, mobile dialogue actions fit in one readable row, first-time desktop Character Hall keeps the create action visible inside the viewport, steady desktop World Hub HUD footprint is smaller, active skill preview/dialogue no longer reveal bottom-edge clutter, session menu pause actions now fit desktop/tablet/mobile without clipped settings or debug-like scrollbar chrome, and the world hub no longer lets the target dummy compete with the Gate Keeper objective signal until combat focus is active; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
...
- Next implementation task after this fix: continue from the viewport/profile model; next visible target should improve Character Hall selected-state action placement or remaining world moment-to-moment feedback using owner-level layout/profile helpers, not one-off raw `Screen.width/height` tuning.
