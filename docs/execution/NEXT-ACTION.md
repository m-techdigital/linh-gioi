...
Extract reusable density profiles for compact panels, status rows, and Character Hall cards so future UI screens do not drift into one-off spacing. Marker ready from the completed evidence refresh: `LGO_RUNTIME_UI_CHARACTER_HALL_CONTENT_DENSITY_EVIDENCE_REFRESH_READY`.
