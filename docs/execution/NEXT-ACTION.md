...
Review Player đã bác hai giả định nguồn. V8 dùng 12 layer KRA nhưng các layer là compositing patch/occluder, gây nhân đôi tay. V9 dùng mười cutout độc lập nhưng bbox-center registration làm hở khớp. V10 chuyển sang trục alpha đầu–cuối đo tự động và cải thiện rõ assembly, không có pose-local scale/offset. Gate v12 đo trực tiếp chín joint trong source-space: cổ, hai khuỷu, hai gối và vai xa pass ≤2px; vai gần fail 2,35px, hông gần 17,75px và hông xa 14,42px, đều quy về coverage của `torso-hips`. Capture v10 có 12/14 frame Player; hai frame cuối chụp editor sau khi Player thoát và cadence chưa chứng minh từng state bằng ảnh có nhãn. Evidence: `build/character-model-architecture-review-01/skeletal-2d-player-probe/runtime-v10/{runtime-report,visual-review}.json` và `contact-sheet.png`.

Action kế tiếp là `LGO_SKELETAL_2D_SOURCE_REGISTRATION_CONTRACT_02`: giữ nguyên sáu joint đã pass; re-author hoặc thêm source-owned overlap/cap cho `torso-hips` tại đúng ba socket fail, rồi chạy lại gate 2px. Sau đó implement seam trang phục–body thật và capture theo state marker thay vì timer ngoài. Không quay lại bbox center, chỉnh pixel/joint offset hoặc sinh raster theo pose. Sau khi continuity pass mới nối đủ năm slot, mixed loadout và một unseen item để đo chi phí tái dùng.
