...
Audit remaining screen-specific padding assignments in `M4PlayableClientController` and move safe repeated values into `RuntimeUiLayoutProfile` without changing visual values. Marker ready from the completed evidence refresh: `LGO_RUNTIME_UI_FACTORY_PADDING_HELPER_EVIDENCE_REFRESH_READY`.
...
`LGO-RUNTIME-UI-FACTORY-PADDING-HELPER-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-RUNTIME-UI-CONTROLLER-PADDING-PROFILE-CANDIDATE-AUDIT-v1.0`; the next useful batch is moving safe screen-specific controller padding candidates into `RuntimeUiLayoutProfile`.
