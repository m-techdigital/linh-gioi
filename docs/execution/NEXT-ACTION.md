...
Next valid work: execute source visual polish, not another missing-target authoring loop. Keep the fixed candidate directories under `/Users/minhdc/Projects/Design/LGO-Selected-2D-Source-v1/class-work-in-progress/phap-lv001/pose-matched-layer-authoring-v1`, preserve provenance and `DO-NOT-PACK`, and improve the visible board result before Player pack. The first blocker is `outer_top` run/jump: it is body-aware and in-slot, but still reads as body-painted/sleeveless instead of matching the idle Pháp top with white sleeves. After visual source fixes, regenerate A/B source boards and mixed/off-slot boards; only then consider a review-only Player pack.
