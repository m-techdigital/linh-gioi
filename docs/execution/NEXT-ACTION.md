...
- Current reason: mobile guided object labels now show compact object names while desktop/tablet keep full `Mục tiêu` labels, reducing world-label clutter near interactables; runtime profile screenshots were reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
