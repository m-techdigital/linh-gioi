...
- Current reason: skill preview feedback now persists in the visible combat panel and highlights the active preview button, so Shadow Bind evidence connects HUD text to the red world telegraph; screenshot evidence was reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
...
- Next implementation task after this fix: choose the next compact player-visible slice, likely local combat timing/reward feel, Character Hall final-art parity, or another screenshot-proven UI/UX issue; avoid heavy assets and one-off validators unless a real gate needs them.
