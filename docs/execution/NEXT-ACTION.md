...
Việc tiếp theo có thể làm ngay là sửa source ownership của 7/16 pack fail trong `build/source-pose-design-audit-v2/semantic-audit.json`: Pháp nam Lv1/Lv10, Pháp nữ Lv1/Lv10, Cơ nam Lv10, Cơ nữ Lv1/Lv10. Bám turnaround/equipment reference gốc; tách đúng module có thể tháo, trả pixel áo/quần/giáp về slot tương ứng, và giữ một body/action authority. Mỗi pack phải qua board full compose, 10 trạng thái tháo, sáu pose và semantic audit trước khi đóng atlas.

Sau khi sửa nguồn theo batch, build/capture một Player có đổi Kiếm/Pháp/Cơ/Linh bằng `F`, đổi giới, đổi Lv1/Lv10 và phối chéo item trên cùng actor. Review ảnh lớn cho idle, đủ `contact A → run A → contact B → run B`, jump tuck và từng slot tháo/mặc. Không claim hoàn thành từ test/count; chỉ nâng trạng thái sau visual review thực tế. Võ div4 và registered outfit giữ nguyên; chưa có pack Võ nữ thì không ghép giả hoặc mở presentation thứ hai.
