...
Optimize the largest safe runtime assets only when quality can be preserved: start with transparent PNG candidates that exceed their visual role needs, update manifest hashes/sizes, and verify screenshots before deleting or replacing anything.
...
Next allowed action: optimize safe runtime asset sizes after the inventory pass. The visual evidence harness records explicit review checklist categories for every checkpoint while still refusing to claim visual PASS from capture/build alone.
...
`LGO-RUNTIME-ASSET-SIZE-INVENTORY-PASS-v1.0` is closed. Continue with `LGO-RUNTIME-ASSET-OPTIMIZATION-PASS-v1.0`.
