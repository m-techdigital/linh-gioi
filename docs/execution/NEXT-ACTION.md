## Active — FIX_REQUIRED: hoàn thiện tab Nhân vật theo design đã duyệt — 2026-09-14
...
Shared layout tổng đã qua Player gate mới tại `build/map01a-five-tab-layout-runtime-v4/{pc,mobile,tablet}/`: một shell, một hàng năm tab, body hai cột và detail-right giữ cùng composition ở cả ba viewport. Trạng thái từng screen vẫn là `FIX_REQUIRED`; active duy nhất là `Nhân vật`.

### Batch kế tiếp

- Đối chiếu duy nhất `01-nhan-vat-nam-tab-compact-APPROVED.png`; hoàn thiện hierarchy, spacing, typography, equipment rail, actor stage, HP/MP và detail-right của tab `Nhân vật`.
...
- Chỉ khi Player `Nhân vật` đạt ở đủ ba viewport mới chuyển `Rương đồ`; sau đó mới `Kỹ năng → Tiềm năng → Linh thú`.
...
- Build Player một lần sau batch; capture trạng thái mặc định/chọn món/khóa món của `Nhân vật` ở ba viewport và xem trực tiếp trước khi cập nhật trạng thái.
