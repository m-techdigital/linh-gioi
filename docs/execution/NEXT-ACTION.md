...
Authority: `docs/execution/LGO-RIGID-OUTFIT-PILOT-GOAL.md`. Execution plan: `docs/superpowers/plans/2026-09-15-rigid-outfit-pilot-production.md`. Single-image/SAM2/inpaint assessment: `docs/art/LGO-SINGLE-IMAGE-MODULAR-RIG-ASSESSMENT-2026-09-15.md`.
...
Blender 4.5.13 LTS đã qua checksum/codesign và fresh-process rigid RGBA reopen probe. Codex tiếp tục tạo đúng một neutral male/female source revision bằng volumetric rigid objects và xuất board toàn thân. Gate này chỉ duyệt identity, góc 12°, tỷ lệ, stance, body silhouette và joint zones; chưa dựng outfit hoặc tích hợp source mới vào Unity.

Đường duy nhất: pinned Blender → neutral design gate → rigid body/motion → outfit #1 → RGBA export → Unity Final #1. Source mới không dùng geometry/pixel/code của mannequin cũ, không flat card, capsule, weight hoặc deformation. V9 mask chỉ làm QA contour; Grounded-SAM2/Krita/ComfyUI không nằm trên critical path vì chưa chứng minh hidden geometry hoặc headless capability trên máy này. Nếu revision 02 lặp lỗi cấu trúc, tự dừng `BLOCKED_BY_ASSET`; không thử revision 03.

Apple Vision probe mới không đổi bước kế tiếp: nó nhận được 19 joint nhưng lệch core RMS khoảng 44 px và chỉ trả whole foreground. Không dùng Vision để cắt part; không thêm Moho/Rive; không tạo angle-dependent joint sprite. Evidence tổng hợp: `build/rigid-outfit-pilot/capability-evidence/evidence-index-v1.json`.

Khi continuous Final #1 đạt, dừng chờ owner review trước outfit #2 đúng chỉ đạo đã khóa.
