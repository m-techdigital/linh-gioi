...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, north-star and mobile/tablet UI reference sheets anchor layout decisions, and `RuntimeUiOverflowGuard` centralizes bounded action rows, scroll regions, overlay placement, and responsive action columns. World dialogue now has explicit header/body/footer ownership, body-only scrolling, viewport-centered overlay placement with equal top/bottom insets, and a long-text evidence checkpoint. Session menu mobile centers through the same overlay base, Character Hall mobile create/dock overlays now use shared horizontal/vertical placement anchors instead of one-off absolute constants, and Character Hall create/enter-world buttons now use shared semantic tiers instead of selected-state-only sizing constants; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
