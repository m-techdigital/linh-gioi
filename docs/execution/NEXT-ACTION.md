...
- Checkpoint v6 đã dựng sẵn topology 600×520 và detail-right theo hero/current level/current/next/cost; `Võ/Kiếm/Pháp/Cơ/Linh` chỉ bind profile/data/icon/text/state vào cùng object tree.
- Player: `build/map01a-character-hub-potential-fidelity-player-v6/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/map01a-character-hub-potential-fidelity-runtime-v6/{pc,mobile,tablet}/{potential-default,potential}.png`; 9 frame/profile, `usesOsMouseOrKeyboard=false`.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v6, tập trung tỷ lệ topology/core figure/năm node-value-add và hierarchy current/next/cost ở detail-right.
...
Evidence:
`build/map01a-character-hub-potential-fidelity-runtime-v6/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- Player v6 `Succeeded`, `errors=0`, `warnings=48`; frozen diff/change budget/`git diff --check` pass.
