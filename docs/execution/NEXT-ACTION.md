...
Review whether evidence-only UI state toggles should gain a small helper after World HUD component boundary cleanup. Marker ready from the completed source refactor: `LGO_WORLD_HUD_COMPONENT_BOUNDARY_AUDIT_READY`. Focus on preventing one-off screenshot state flags from accumulating while keeping gameplay runtime behavior unchanged.
