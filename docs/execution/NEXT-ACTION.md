...
`LGO_VO_LV1_PAPERDOLL_ATLAS_RUNTIME_READY`: Võ Lv1 đã có runtime resource `LGOClasses/VoLv1PaperDollAtlas.json` mô tả 13 part paper-doll và 3 skill cue theo slot/anchor/shape, không dùng source image và không 3D. Controller render overlay data-driven trên PC thật tại frame hoàn tất training; visual manifest có `runtimeVoLv1PaperDollAtlasSnapshot` với `parts=13`, `slot=OuterShirt`, `anchor=Chest`, `skillCue=vo_lv1_first_skill_trail`, `pose=vo_lv1_training_complete`, `active=True`. Fix kèm theo cho capture: visual runner chạy capture sequence đồng bộ trong `Start()` và tự render `Camera.Render()` vào RenderTexture, tránh kẹt coroutine yield trong macOS Player evidence. Đây vẫn là primitive/blockout atlas để kiểm chứng pipeline tách đồ/anchor/skill, chưa phải art production giống concept. Evidence: Unity EditMode `total=147 passed=146 failed=0 skipped=1`, onboarding smoke PASS, macOS Player build Succeeded, Player visual capture PASS 20 frame, runtime smoke matrix PASS, visual evidence matrix PASS, no-3D/no-source-image PASS, frozen diff audit PASS. Next action: thay primitive atlas Võ bằng bitmap/spritesheet approved hoặc làm motion/anchor chi tiết hơn cho Võ trước khi nhân pattern sang Kiếm/Pháp/Cơ/Linh.
