## Active — owner visual review màn Yêu cầu khôi phục mật khẩu — 2026-09-14
...
`NEED_HUMAN_VISUAL_REVIEW`. Entry/Login, Character Select, Chọn máy chủ và Đăng ký đã đạt layout gate. Screen Yêu cầu khôi phục đã có một canonical candidate, runtime và evidence ba profile; đây là technical checkpoint, chưa phải owner approval.

Screen active duy nhất là visual review **Quên mật khẩu · Yêu cầu khôi phục**. Candidate: `/Users/minhdc/Projects/Design/LGO-2D-UI-Owner-Demos-2026-09-13/redesign-v9-password-recovery-request/01-password-recovery-request-CANONICAL-CANDIDATE.png`; Player `build/map01a-password-recovery-request-player-v1/LinhGioiOnline.app`; evidence `build/map01a-password-recovery-request-runtime-v1/{pc,mobile,tablet}/`. Runtime chỉ có một field/một CTA, dùng shared auth-flow base và báo backend chưa kết nối; không giả gửi mã. Sau khi owner duyệt mới đổi candidate thành canonical và mở design gate **Xác minh mã**, rồi mới **Đặt mật khẩu mới**. Không code screen kế tiếp song song, không quay về quy trình cũ, không resume class/pose/wardrobe/source, không chạy class capture, không rollback code class và không đổi frozen surfaces.
...
Next valid work: Map01A/UI-only, ưu tiên detail stats card/state badge hoặc login/HUD/dialog theo design owner. Không resume class/wardrobe/pose/source, không rollback class code, không icon giả/random/generated; visible batch phải có Player screenshot review bằng mắt.
...
## Next Map01A UI polish after supplies list card — 2026-09-13

Continue Map01A product UI/UX polish using shared skin/base helpers and approved/runtime assets only. `Vật phẩm` now has a left list card and right detail card; next safe work should improve real item/equipment visual richness, inventory/detail hierarchy, login/HUD/dialogue polish, or prepare an approved item-thumbnail design board. Do not add fake/generic icons, random item/class art, production auth/storage, or class build/capture loops.
...
## Current next action — after inventory three-column checkpoint (2026-09-13)
...
## Next — sau khi khóa screen Rương đồ theo canonical design — 2026-09-14

`CONTINUE`, chỉ Map01A/UI. Screen `Rương đồ` đã khóa layout theo design duy nhất `redesign-v4-five-tabs/02-ruong-do-phan-loai-doc-tab-compact-APPROVED.png`. Evidence hiện hành duy nhất là `build/map01a-bag-screen-runtime-v4/{pc,mobile,tablet}/`; Player `build/map01a-bag-screen-player-v3/LinhGioiOnline.app`. Giữ rail phân loại icon dọc, lưới vuông năm cột, search, hai action footer và detail món bên phải. Không quay lại capture Rương đồ cũ hoặc vi chỉnh màn này khi chưa có feedback owner mới.

Screen active tiếp theo duy nhất là `Kỹ năng`, canonical `03-ky-nang-five-tab-APPROVED.png`. Trước khi sửa runtime phải tạo plan/audit toàn màn theo shell → rail phân loại → skill graph ba hàng → equipped-skill strip → detail-right → asset → typography; chốt asset provenance và trạng thái read-only trước code. Không sửa Tiềm năng/Linh thú trước khi Kỹ năng qua gate. Không resume class/pose/wardrobe/source, không rollback code class và không đổi frozen surfaces.
