...
Audit the remaining Character Hall helpers after polish and decide whether another small stateless factory extraction is useful or whether the next highest-value work should move to World HUD/Login visual quality. Marker ready from evidence refresh: `LGO_CHARACTER_HALL_V3B_VISUAL_POLISH_EVIDENCE_REFRESH_READY`.
...
`LGO-CHARACTER-HALL-V3B-VISUAL-POLISH-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-RUNTIME-UI-FACTORY-CHARACTER-HALL-CLEANUP-FOLLOWUP-v1.0`; the next useful batch is deciding and applying the next reuse cleanup or visual target from current screenshots.
