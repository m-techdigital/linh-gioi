...
Map the remaining World HUD component boundaries after combat button mobile responsive evidence refresh. Marker ready from the completed profile evidence pass: `LGO_COMBAT_BUTTON_MOBILE_RESPONSIVE_EVIDENCE_READY`. Focus on reusable HUD grouping, responsive shell ownership, and evidence-friendly UI states before extracting more code.
