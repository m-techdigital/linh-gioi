...
- Current reason: responsive layout now separates screen form-factor classification from UI Toolkit panel-space sizing, re-applies when the resolved viewport width/height changes, runtime loads `LGORuntimePanelSettings` from `Resources`, and desktop/tablet/mobile login screenshots show logo, server row, and CTA inside the viewport without claiming `VISUAL_RUNTIME_PASS`.
...
- Next implementation task after this fix: continue from a panel-space responsive model; next visible target should use current profile evidence to reduce mobile/tablet HUD footprint or improve world/combat readability before touching unrelated hygiene.
