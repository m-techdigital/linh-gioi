...
1. Commit/push batch map layer budget/minimap Chapter 1 sau khi Unity EditMode, onboarding smoke, Player build/capture, visual review và source validators pass.
...
Chưa có blocker. Visual capture hiện bắt được world stage, HUD world-space, minimap/route overlay, scene-beat metadata và character base snapshot bằng camera render; manifest lưu `hudSnapshot`, `productionSceneBeatSnapshot`, `runtimeMapSnapshot`, `runtimeCharacterBaseSnapshot`, `runtimeEquipmentSnapshot`, `runtimeAnimationSnapshot` để kiểm copy/trạng thái. Ảnh source cũ đã bị loại khỏi source tree và có validator riêng để ngăn tái nhập nhầm. Shadow Slime micro-slice và map layer budget hiện được kiểm bằng state/catalog test, controller snapshot, smoke runner và visual manifest; chưa có blocker.
