...
Document RuntimeUiSkin helper ownership after the completed adoption/evidence series. Marker: `LGO_RUNTIME_UI_SKIN_ADOPTION_EVIDENCE_REFRESH_READY`. Keep the guide focused on reusable UI helper boundaries, adoption rules, and next safe cleanup targets so future screens do not reintroduce duplicated style blocks.
