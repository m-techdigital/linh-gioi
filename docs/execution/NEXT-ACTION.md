> **Gate Map01A playable visual slice — composition v1, 2026-09-10:** functional Q01–Q09 vẫn pass; camera/ground framing đã đổi sang 3,8/1,5 và 21 placement cây/trúc/hoa/đá được tái dùng từ atlas hiện có, không tăng texture count hoặc texture budget. Capture `build/map01a-composition-v1/capture/` có 78 artifact/profile cho mobile/tablet/PC simulation; arrival, market/dialogue, combat và portal đã review trực tiếp. Trạng thái `MAP01A_PLAYABLE_VISUAL_SLICE_PASS / ART_PRODUCTION_DRAFT`; chưa claim illustration final, thiết bị vật lý, persistence/backend hoặc Map01B.
>
> **Next:** chỉ làm `CLASS-VO-01` Lv1–30. Kế thừa base nam/nữ, 10 slot và `TwoDSkeletalPaperDollRig`; đóng equip/unequip xuyên level cùng idle/walk/run/jump/basic attack/`Liên Quyền`/hit feedback trong một batch Player-visible. Khóa Kiếm draft hiện có, không mở Pháp/Cơ/Linh trước khi Võ qua gate.
