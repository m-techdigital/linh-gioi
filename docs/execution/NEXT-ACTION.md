...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, World labels/prompts use one World viewport profile, mobile dialogue actions fit in one readable row, first-time desktop Character Hall keeps the create action visible inside the viewport, steady desktop World Hub HUD footprint is smaller, active skill preview/dialogue no longer reveal bottom-edge clutter, and session menu pause actions now fit desktop/tablet/mobile without clipped settings or debug-like scrollbar chrome; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
...
- Next implementation task after this fix: continue from the viewport/profile model; next visible target should improve Character Hall/world moment-to-moment feedback or remaining tablet/desktop HUD hierarchy using owner-level layout/profile helpers, not one-off raw `Screen.width/height` tuning.
