...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, north-star and mobile/tablet UI reference sheets anchor layout decisions, `RuntimeUiOverflowGuard` centralizes bounded action rows/scroll regions, World dialogue now has explicit header/body/footer ownership with body-only scrolling and a long-text evidence checkpoint, first-time Character Hall hides the invalid enter-world action until a character exists, mobile selected Character Hall uses a compact bottom action dock, and session menu pause actions fit desktop/tablet/mobile; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
...
- Next implementation task after this fix: keep following the target case matrix in `docs/design/RUNTIME-UI-RESPONSIVE-LAYOUT-HELPER-REVIEW-v1.0.md`; next likely player-visible gap is Login mobile/tablet composition scale or Character Hall desktop/tablet composition density, using bounded containers and profile evidence instead of one-off raw `Screen.width/height` tuning.
