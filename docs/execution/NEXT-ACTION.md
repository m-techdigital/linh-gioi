...
Audit remaining controller-local style constants after runtime UI style ownership evidence refresh. Marker ready from the completed evidence pass: `LGO_RUNTIME_UI_STYLE_OWNERSHIP_EVIDENCE_REFRESH_READY`. Consolidate only repeated non-stateful constants or helper calls that clearly reduce drift across login, character hall, world HUD, and session menu.
...
`LGO-RUNTIME-UI-STYLE-OWNERSHIP-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-RUNTIME-UI-CONTROLLER-STYLE-CONSTANTS-AUDIT-v1.0`; the next useful batch is reducing remaining controller-local style constants without moving gameplay state.
