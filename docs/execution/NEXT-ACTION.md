...
- Current reason: guided interaction now keeps Đá Luyện inactive during step 1, so standing near the stone no longer shows a contradictory `F/Space - Luyện` prompt before meeting Người Giữ Cổng; screenshot evidence was reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
...
- Next implementation task after this fix: choose the next compact player-visible slice, likely movement/input feel, post-dialogue Đá Luyện prompt clarity, or another screenshot-proven UI/UX issue; avoid heavy assets and one-off validators unless a real gate needs them.
