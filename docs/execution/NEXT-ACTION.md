...
Continue UI foundation cleanup by auditing direct left/right/top/bottom padding assignments inside `RuntimeUiFactory`/`RuntimeUiSkin` helpers and routing suitable repeated cases through `RuntimeUiSkin.ApplyPadding`. Marker ready from the completed evidence refresh: `LGO_RUNTIME_UI_SESSION_MENU_PADDING_PROFILE_EVIDENCE_REFRESH_READY`.
...
`LGO-RUNTIME-UI-SESSION-MENU-PADDING-PROFILE-EVIDENCE-REFRESH-v1.0` is evidence-ready. Continue with `LGO-RUNTIME-UI-FACTORY-PADDING-HELPER-COVERAGE-AUDIT-v1.0`; the next useful batch is tightening remaining factory/helper padding duplication.
