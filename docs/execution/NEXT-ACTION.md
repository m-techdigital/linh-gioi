...
- Checkpoint v9 thay topology ghép primitive bằng một template tĩnh 600×520 có sẵn vòng/đường/node/value/add/core; class chỉ bind icon/tên/value/selection trên overlay dùng chung. Import giữ đúng native size bằng `nPOTScale: 0`.
- Player: `build/map01a-character-hub-potential-template-player-v9/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/map01a-character-hub-potential-template-runtime-v9/{pc,mobile,tablet}/{potential-default,potential}.png`; 9 frame/profile, `usesOsMouseOrKeyboard=false`.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v9, tập trung template topology cố định, alignment icon/tên/value trên năm node và hierarchy current/next/cost ở detail-right.
...
Evidence:
`build/map01a-character-hub-potential-template-runtime-v9/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- Player v9 `Succeeded`, `errors=0`; frozen diff/`git diff --check` pass. Runtime asset 53.995 byte và toàn bộ skin khóa native-size import.
