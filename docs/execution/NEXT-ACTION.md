...
- Checkpoint v12 giữ polish topology v11 và đóng băng catalog/state: collection không lộ backing array, ID/index được kiểm lúc khởi tạo, selection skill/potential tách biệt đủ năm class. Không đổi asset/class art.
- Player: `build/character-hub-class-state-player-v12/LinhGioiOnline.app`.
- Evidence hiện hành duy nhất: `build/character-hub-class-state-runtime-v12/{pc,mobile,tablet}/{potential-default,potential}.png`; lần lượt 1280×720, 1024×768, 1600×720; 9 frame/profile, `usesOsMouseOrKeyboard=false`. Capture bằng `python3.12 tools/capture_lgo_character_hub.py ...`; không nhận evidence nếu ba profile dùng sai kích thước.
- Không phát triển class/pose/wardrobe/source art trong task này; không phục hồi renderer cũ hoặc dựng UI theo class.

## Next task

1. Owner review v12, tập trung độ sâu vòng rune/lõi thiền, alignment icon/tên/value trên năm node và hierarchy current/next/cost ở detail-right trên đủ ba viewport thật.
...
Evidence:
`build/character-hub-class-state-runtime-v12/{pc,mobile,tablet}/{potential-default,potential}.png`
...
- Player v12 `Succeeded`, `errors=0`; 50 warning CS0618 là deprecation hiện hữu. Capture đúng PC/tablet/mobile; full EditMode `286/287` pass, 1 ignored; frozen diff/`git diff --check` pass. Runtime asset 59.678 byte và toàn bộ skin khóa native-size import.
