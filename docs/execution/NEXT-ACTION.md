...
- Current reason: responsive viewport work now has a canonical UI `RuntimeViewportMetrics` owner, north-star and mobile/tablet UI reference sheets anchor layout decisions, and `RuntimeUiOverflowGuard` centralizes bounded action rows, scroll regions, compact scroll chrome, overlay placement, modal body/footer regions, and responsive action columns. World dialogue now routes body/footer through shared modal primitives with body-only scrolling, viewport-centered overlay placement, equal top/bottom insets, styled compact scrollbar chrome, a long-text evidence checkpoint, and a shared responsive speaker header with the V3B Gate Keeper portrait. Session menu mobile centers through the same overlay base, and Character Hall has begun moving from screen-specific composition to shared demo-aligned contracts: mobile selected state uses a full-safe shell with header/prose hidden, empty state no longer renders selected placeholder hero, and create actions route through shared modal body/footer plus responsive columns; screenshots were reviewed without claiming `VISUAL_RUNTIME_PASS`.
