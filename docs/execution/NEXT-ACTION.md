...
- Current reason: skill preview now has a dedicated `skill-shadow-bind-preview.png` checkpoint, cleaner `Diễn tập an toàn` copy, and a reset preview state so Trói Bóng evidence shows the warning telegraph without stale target-dummy hit feedback; screenshot evidence was reviewed, but no `VISUAL_RUNTIME_PASS` is claimed.
...
- Next implementation task after this fix: choose the next compact player-visible slice, likely World Hub empty-space richness, local combat timing polish, skill button state clarity, or Character Hall final-art parity; avoid heavy assets and one-off validators unless a real gate needs them.
