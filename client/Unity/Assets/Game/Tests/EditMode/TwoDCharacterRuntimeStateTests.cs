...
namespace LinhGioi.Tests.EditMode
{
    public sealed class TwoDCharacterRuntimeStateTests
    {
        ...
        public void EntryCaptureFlagRunsPreviewWithoutSuppressingEntryOverlay()
        {
            ...
            Assert.That(CongDongLamMap01AArtPreview.ShouldCaptureProductAccountStatesForArgs(recoveryNewPasswordArgs), Is.False);
            var productAccountStateArgs = new[] { "LinhGioiOnline", "--lgo-map01a-password-recovery-new-password-capture", "--lgo-product-account-states-capture" };
            Assert.That(CongDongLamMap01AArtPreview.ShouldCaptureProductAccountStatesForArgs(productAccountStateArgs), Is.True);
        ...
        public void SkillRegisteredArtworkBatchHasSeparateContentSprites()
        {
            ...
            try
            {
                ...
                Assert.That(CharacterHubClassCatalog.Profiles.Sum(profile => profile.Skills.Count), Is.EqualTo(45));
                foreach (var profile in CharacterHubClassCatalog.Profiles)
