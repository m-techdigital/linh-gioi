...
namespace LinhGioi.Tests
{
    public sealed class TwoDOnboardingStateTests
    {
        ...
        [Test]
        public void VoLv1PaperDollAtlasLoadsSlotPartsAnchorsAndSkillCueFromRuntimeResource()
        {
            ...
            Assert.AreEqual("vo", atlas.classId);
            Assert.That(atlas.parts.Length, Is.GreaterThanOrEqualTo(10));
            Assert.That(atlas.skillCues.Length, Is.GreaterThanOrEqualTo(3));
            StringAssert.Contains("VoLv1PaperDollAtlas", snapshot);
            StringAssert.Contains("resource=LGOClasses/VoLv1PaperDollAtlas", snapshot);
            StringAssert.Contains("parts=", snapshot);
            StringAssert.Contains("slot=OuterShirt", snapshot);
            StringAssert.Contains("anchor=Chest", snapshot);
            StringAssert.Contains("slot=Gloves", snapshot);
            StringAssert.Contains("anchor=Hand_R", snapshot);
            StringAssert.Contains("skillCue=vo_lv1_first_skill_trail", snapshot);
        ...
        public void VoLv1ApprovedRuntimeArtManifestDefinesLv1To30CellMap()
        {
            ...
            StringAssert.Contains("cells=13", snapshot);
            StringAssert.Contains("approvedCell=chest_panel cell=torso_outer_vo_lv1", snapshot);
            StringAssert.Contains("approvedCell=inner_shadow cell=base_torso_male", snapshot);
            StringAssert.Contains("approvedCell=pants_shadow_l cell=pants_vo_lv1", snapshot);
            StringAssert.Contains("approvedCell=pants_shadow_r cell=pants_vo_lv1", snapshot);
            ...
            StringAssert.Contains("approvedCell=vo_lv1_first_skill_edge cell=skill_vo_lv1_palm_trail", snapshot);
            StringAssert.Contains("approvedCell=vo_lv1_first_skill_impact cell=skill_vo_lv1_palm_burst", snapshot);
        ...
        public void RuntimeControllerUsesApprovedVoLv1ArtForMappedAtlasCells()
        {
            ...
            try
            {
                ...
                var torsoUnderlay = GameObject.Find("LGO 2D Player Vo AtlasCell base_torso_male OuterShirt inner_shadow");
                Assert.IsNotNull(torsoUnderlay);
                var torsoUnderlayRenderer = torsoUnderlay.GetComponent<SpriteRenderer>();
                StringAssert.Contains("ApprovedRuntimeArt", torsoUnderlayRenderer.sprite.name);
                var torsoUnderlayWorldSize = Vector2.Scale(torsoUnderlayRenderer.sprite.bounds.size, torsoUnderlay.transform.localScale);
                Assert.That(torsoUnderlayWorldSize.x, Is.InRange(0.27f, 0.34f));
                Assert.That(torsoUnderlayWorldSize.y, Is.InRange(0.34f, 0.42f));
                var leftPants = GameObject.Find("LGO 2D Player Vo AtlasCell pants_vo_lv1 PantsOrSkirt pants_shadow_l");
                Assert.IsNotNull(leftPants);
                var leftPantsRenderer = leftPants.GetComponent<SpriteRenderer>();
                StringAssert.Contains("ApprovedRuntimeArt", leftPantsRenderer.sprite.name);
                var leftPantsWorldSize = Vector2.Scale(leftPantsRenderer.sprite.bounds.size, leftPants.transform.localScale);
                Assert.That(leftPantsWorldSize.x, Is.InRange(0.13f, 0.19f));
                Assert.That(leftPantsWorldSize.y, Is.InRange(0.28f, 0.36f));
                var rightPants = GameObject.Find("LGO 2D Player Vo AtlasCell pants_vo_lv1 PantsOrSkirt pants_shadow_r");
                Assert.IsNotNull(rightPants);
                var rightPantsRenderer = rightPants.GetComponent<SpriteRenderer>();
                StringAssert.Contains("ApprovedRuntimeArt", rightPantsRenderer.sprite.name);
                var rightPantsWorldSize = Vector2.Scale(rightPantsRenderer.sprite.bounds.size, rightPants.transform.localScale);
                Assert.That(rightPantsWorldSize.x, Is.InRange(0.13f, 0.19f));
                Assert.That(rightPantsWorldSize.y, Is.InRange(0.28f, 0.36f));
                ...
                var skillEdge = GameObject.Find("LGO 2D Player Vo AtlasCell skill_vo_lv1_palm_trail SkillCue vo_lv1_first_skill_edge");
                Assert.IsNotNull(skillEdge);
                var skillEdgeRenderer = skillEdge.GetComponent<SpriteRenderer>();
                StringAssert.Contains("ApprovedRuntimeArt", skillEdgeRenderer.sprite.name);
                var skillImpact = GameObject.Find("LGO 2D Player Vo AtlasCell skill_vo_lv1_palm_burst SkillCue vo_lv1_first_skill_impact");
                Assert.IsNotNull(skillImpact);
                var skillImpactRenderer = skillImpact.GetComponent<SpriteRenderer>();
                StringAssert.Contains("ApprovedRuntimeArt", skillImpactRenderer.sprite.name);
                var skillImpactWorldSize = Vector2.Scale(skillImpactRenderer.sprite.bounds.size, skillImpact.transform.localScale);
                Assert.That(skillImpactWorldSize.x, Is.InRange(0.22f, 0.34f));
                Assert.That(skillImpactWorldSize.y, Is.InRange(0.18f, 0.30f));
        ...
        public void RuntimeControllerRendersVoLv1PaperDollAtlasOverlayForPlayerEvidence()
        {
            ...
            try
            {
                ...
                StringAssert.Contains("VoLv1PaperDollAtlas", controller.RuntimeVoLv1PaperDollAtlasSnapshot);
