...
- Scope hiện hành là hoàn thiện Map01A và các UI screen 2D theo design-first. Entry, Character Select, Chọn máy chủ và Đăng ký đã khóa; screen active duy nhất là owner visual review cho `Quên mật khẩu · Yêu cầu khôi phục`. Chỉ sau review mới tới `Xác minh mã`. Không resume class/pose/wardrobe/source, không chạy class Player/capture loop và không rollback class code.
...
- Sau khi một screen đã đủ canonical, contract, Player evidence ba profile, visual audit và gate kỹ thuật, phải checkpoint rồi chuyển screen active trong `NEXT-ACTION.md`/`PROJECT-STATE.md`; không quay về quy trình cũ, không vi chỉnh lại screen đã khóa nếu chưa có regression hoặc feedback mới có evidence.
...
- Character Hub dựng topology Skill, Tiềm năng và Linh thú đúng một lần từ shared base. Riêng Tiềm năng, một vector base phải sở hữu sẵn vòng ngoài, đường nối, core, toàn bộ khung tròn, ô giá trị, ô cộng điểm và dấu cộng của năm node; button/node phía trên chỉ là interaction/data overlay trong suốt. Năm định nghĩa thuộc tính Tiềm năng dùng một catalog bất biến chung; class chỉ giữ recommendation/selection/state thực sự khác nhau. Đổi class chỉ bind profile/data/icon/text/state vào các node có sẵn; không xóa panel, dựng lại vòng tròn/đường nối hoặc tạo layout riêng theo class. Tiềm năng và Skill là hai topology/component riêng, còn class chỉ cung cấp dữ liệu tương ứng.
...
- UI shared component governance — owner lock 2026-09-13:
  ...
  - Design demo owner upload là visual reference cho toàn bộ login/entry, character select, HUD, inventory/bag, storage/chest, fashion/wardrobe, dialog và item-detail; không follow máy móc 100% khi còn bất hợp lý, nhưng mọi khác biệt lớn phải có lý do UX/sản phẩm và evidence. không dựng khung thô chỉ để có chức năng: UI phải đi theo visual language, spacing, hierarchy, màu/viền/CTA và trạng thái tương tác của design 2D hiện tại.
  - Áp dụng cho toàn bộ design owner mới upload, không chỉ một màn riêng lẻ như login. Khi các màn cùng visual language hoặc cùng interaction pattern, chốt shell/base/shared component trước rồi triển khai các flow dựa trên base đó để tránh mỗi màn một hệ UI.
  - Character hub dùng một hàng tab gọn có thể mở rộng, hiện gồm `Nhân vật`, `Rương đồ`, `Kỹ năng`, `Tiềm năng`, `Linh thú`; không giữ tab Hành trang/Rương đồ cũ song song. Nhân vật và Rương đồ dùng đúng bố cục hai cột nội dung + item-detail bên phải; Rương đồ phân loại bằng rail dọc trong cột nội dung. Khi chọn món từ túi hoặc từ trang bị nhân vật, detail đồ luôn ở panel bên phải theo design đã chốt.
  - modal/dialog/card/tab/button/detail panel dùng base chung trong shared skin/helper trước khi tạo biến thể riêng. Không tạo helper skin song song kiểu `StyleModalDialog`, `BuildCardPanel`, `CreateDetailPanel` trong partial runtime; nếu cần biến thể thì thêm wrapper/base vào shared skin trước. Nếu UI/UX giống nhau, cập nhật base/shared component trước rồi mới bind data/state/action từng màn. Nếu hai UI/UX giống nhau mà cần khác hành vi, tách data/state/action, không copy nguyên layout/skin thành hệ mới. Helper ngoại lệ như `InventoryPanel` chỉ được nằm trong partial sở hữu flow; không copy lại tên helper sang màn khác để né validator.
